# COMPLETE VYSTIA BESTIARY - T2A ERA STANDARD LOOT
## All Creatures with Correct UOR Body Types, Hue Values, and T2A Loot Tables

---

## BODY ID CORRECTIONS FOR UO:RENAISSANCE
**Important:** Using only body IDs that existed in UO:R (2000)
- No Golem (752) - use Earth Elemental (14)
- No Third Dawn Reaper (301) - use original Reaper (47) or Corpser (8)
- No Giant Bat (317) - use Mongbat (39) or Bird (6)
- No Stone Harpy (788) - use Harpy (30)
- Body 47 IS the original Reaper from T2A (safe to use)
- Body 39 is Mongbat (NOT Lizardman - Lizardman is 35/36)
- Most bodies under ID 200 are safe for UOR

**Note:** This version uses standard T2A loot tables (LootPack.*) instead of custom Vystia items, and T2A-era damage values.

---

# FROSTHOLD & WINTERGUARD CREATURES

### FROST GIANT
- **Body ID:** 76 (Titan) ✓
- **Hue:** 1152 (Ice blue)
- **BaseSoundID:** 609
- **Stats:** STR 400-500, DEX 60-80, INT 40-60
- **HP:** 800-1000
- **Damage:** 16-22 (T2A Titan base)
- **Virtual Armor:** 48
- **Special:** Cold damage 25%, Unprovokable
- **Loot:** LootPack.FilthyRich, LootPack.Gems (8)
- **Taming:** Not tameable

### WINTER WOLF
- **Body ID:** 23 (White Wolf) ✓
- **Hue:** 1150 (Snow white)
- **BaseSoundID:** 0xE5
- **Stats:** STR 120-150, DEX 80-100, INT 35-50
- **HP:** 85-100
- **Damage:** 11-17 (T2A Dire Wolf base)
- **Virtual Armor:** 32
- **Special:** Pack tactics, Cold damage 25%
- **Loot:** LootPack.Average
- **Taming:** Yes - MinTame 85.1, ControlSlots 1

### ICE TROLL
- **Body ID:** 53 (Troll) ✓
- **Hue:** 1153 (Frost white)
- **BaseSoundID:** 461
- **Stats:** STR 200-250, DEX 45-65, INT 45-70
- **HP:** 150-200
- **Damage:** 8-14 (T2A Troll base)
- **Virtual Armor:** 40
- **Special:** Regeneration, Cold damage 25%
- **Loot:** LootPack.Average
- **Taming:** Not tameable

### FROST WRAITH
- **Body ID:** 26 (Wraith) ✓
- **Hue:** 1152 (Ice blue)
- **BaseSoundID:** 0x482
- **Stats:** STR 100-120, DEX 75-95, INT 150-200
- **HP:** 80-100
- **Damage:** 7-11 (T2A Wraith base)
- **Virtual Armor:** 28
- **Special:** Mana drain, Cold damage 50%
- **Loot:** LootPack.Meager
- **Taming:** Not tameable

### FROZEN HORROR
- **Body ID:** 57 (Bone Knight) ✓
- **Hue:** 1151 (Pale ice)
- **BaseSoundID:** 0x48D
- **Stats:** STR 250-300, DEX 50-70, INT 50-70
- **HP:** 200-250
- **Damage:** 15-20 (T2A Bone Knight base)
- **Virtual Armor:** 40
- **Special:** Cold aura, Freeze attack
- **Loot:** LootPack.Rich, LootPack.MedScrolls
- **Taming:** Not tameable

### FROST DRAGON
- **Body ID:** 12 (Dragon) ✓
- **Hue:** 1152 (Ice blue)
- **BaseSoundID:** 362
- **Stats:** STR 796-825, DEX 86-105, INT 436-475
- **HP:** 478-495
- **Damage:** 16-22 (T2A Dragon base)
- **Virtual Armor:** 60
- **Special:** Ice breath, Flight, Magic user
- **Loot:** LootPack.FilthyRich (2), LootPack.Gems (8)
- **Taming:** Not tameable

### ICE SPRITE
- **Body ID:** 58 (Wisp) ✓
- **Hue:** 1150 (Ice white)
- **BaseSoundID:** 466
- **Stats:** STR 50-80, DEX 150-200, INT 100-150
- **HP:** 50-75
- **Damage:** 5-10 (T2A Wisp base)
- **Virtual Armor:** 25
- **Special:** Magic user, Invisibility
- **Loot:** LootPack.Meager
- **Taming:** Not tameable

---

# EMBERLANDS CREATURES

### LAVA HOUND
- **Body ID:** 98 (Hell Hound) ✓
- **Hue:** 1161 (Lava red)
- **BaseSoundID:** 0xE5
- **Stats:** STR 102-150, DEX 81-105, INT 36-60
- **HP:** 100-150
- **Damage:** 11-17 (T2A Hell Hound base)
- **Virtual Armor:** 30
- **Special:** Fire breath (50 damage)
- **Loot:** LootPack.Average
- **Taming:** Yes - MinTame 85.5, ControlSlots 1

### MAGMA TROLL
- **Body ID:** 53 (Troll) ✓
- **Hue:** 1358 (Molten orange)
- **BaseSoundID:** 461
- **Stats:** STR 250-300, DEX 50-70, INT 50-70
- **HP:** 200-250
- **Damage:** 8-14 (T2A Troll base)
- **Virtual Armor:** 35
- **Special:** Fire aura, Regeneration
- **Loot:** LootPack.Average
- **Taming:** Not tameable

### PHOENIX
- **Body ID:** 5 (Eagle) ✓
- **Hue:** 1358 (Fire orange)
- **BaseSoundID:** 0x8F
- **Stats:** STR 504-700, DEX 202-300, INT 504-700
- **HP:** 340-383
- **Damage:** 12-18 (T2A Griffin base)
- **Virtual Armor:** 60
- **Special:** Fire breath (75 damage), Resurrection
- **Loot:** LootPack.Rich, LootPack.Gems
- **Taming:** Not tameable

### VOLCANO WYRM
- **Body ID:** 49 (Ancient Wyrm) ✓
- **Hue:** 1359 (Deep red)
- **BaseSoundID:** 362
- **Stats:** STR 900-1000, DEX 100-120, INT 500-600
- **HP:** 600-700
- **Damage:** 16-22 (T2A Dragon base)
- **Virtual Armor:** 70
- **Special:** Massive fire breath (90 damage)
- **Loot:** LootPack.FilthyRich (2), LootPack.Gems (8)
- **Taming:** Not tameable

### EMBER BAT
- **Body ID:** 39 (Mongbat) ✓
- **Hue:** 1160 (Ember red)
- **BaseSoundID:** 0x270
- **Stats:** STR 30-50, DEX 90-120, INT 10-20
- **HP:** 30-40
- **Damage:** 5-8 (T2A Mongbat base)
- **Virtual Armor:** 15
- **Special:** Swarm tactics, Fire bite
- **Loot:** LootPack.Poor
- **Taming:** Yes - MinTame 60.0, ControlSlots 1

### ASH GOLEM
- **Body ID:** 14 (Earth Elemental) ✓
- **Hue:** 1109 (Ash grey)
- **BaseSoundID:** 268
- **Stats:** STR 200-250, DEX 40-60, INT 40-60
- **HP:** 180-220
- **Damage:** 9-16 (T2A Earth Elemental base)
- **Virtual Armor:** 35
- **Special:** Fire immunity, Ash cloud
- **Loot:** LootPack.Average, LootPack.Meager, LootPack.Gems
- **Taming:** Not tameable

### FLAME SPRITE
- **Body ID:** 58 (Wisp) ✓
- **Hue:** 1360 (Bright flame)
- **BaseSoundID:** 466
- **Stats:** STR 60-90, DEX 120-150, INT 80-100
- **HP:** 40-60
- **Damage:** 5-10 (T2A Wisp base)
- **Virtual Armor:** 20
- **Special:** Fire magic, Ignite, Burn aura
- **Loot:** LootPack.Meager
- **Taming:** Not tameable

---

# WHISPERING SANDS CREATURES

### SAND ELEMENTAL
- **Body ID:** 14 (Earth Elemental) ✓
- **Hue:** 2305 (Sandy tan)
- **BaseSoundID:** 268
- **Stats:** STR 126-155, DEX 66-85, INT 71-92
- **HP:** 150-200
- **Damage:** 9-16 (T2A Earth Elemental base)
- **Virtual Armor:** 34
- **Special:** Sandstorm, Blind
- **Loot:** LootPack.Average, LootPack.Meager, LootPack.Gems
- **Taming:** Not tameable

### DESERT MUMMY
- **Body ID:** 154 (Mummy) ✓
- **Hue:** 2309 (Dusty brown)
- **BaseSoundID:** 471
- **Stats:** STR 200-250, DEX 40-60, INT 40-60
- **HP:** 200-250
- **Damage:** 12-18 (T2A Mummy base)
- **Virtual Armor:** 35
- **Special:** Curse, Disease
- **Loot:** LootPack.Rich, LootPack.MedScrolls
- **Taming:** Not tameable

### GIANT SCORPION
- **Body ID:** 48 (Scorpion) ✓
- **Hue:** 2307 (Desert brown)
- **BaseSoundID:** 397
- **Stats:** STR 73-115, DEX 76-95, INT 16-30
- **HP:** 100-150
- **Damage:** 5-10 (T2A Scorpion base)
- **Virtual Armor:** 28
- **Special:** Greater poison
- **Loot:** LootPack.Meager
- **Taming:** Yes - MinTame 80.0, ControlSlots 1

### SPHINX
- **Body ID:** 30 (Harpy) ✓
- **Hue:** 2306 (Sandstone)
- **BaseSoundID:** 402
- **Stats:** STR 300-400, DEX 100-150, INT 200-300
- **HP:** 350-400
- **Damage:** 5-7 (T2A Harpy base)
- **Virtual Armor:** 45
- **Special:** Riddles, Magic resistance
- **Loot:** LootPack.Rich, LootPack.MedScrolls (2)
- **Taming:** Not tameable

### DESERT HARPY
- **Body ID:** 30 (Harpy) ✓
- **Hue:** 2308 (Sun-bleached)
- **BaseSoundID:** 402
- **Stats:** STR 100-150, DEX 120-160, INT 80-100
- **HP:** 80-120
- **Damage:** 5-7 (T2A Harpy base)
- **Virtual Armor:** 25
- **Special:** Flight, Screech
- **Loot:** LootPack.Meager (2)
- **Taming:** Not tameable

### ANKHEG
- **Body ID:** 52 (Giant Beetle) ✓
- **Hue:** 2304 (Sand yellow)
- **BaseSoundID:** 0x21D
- **Stats:** STR 200-250, DEX 60-80, INT 20-40
- **HP:** 150-200
- **Damage:** 12-18 (T2A Giant Beetle base)
- **Virtual Armor:** 35
- **Special:** Burrow, Acid spit
- **Loot:** LootPack.Average
- **Taming:** Not tameable

---

# SHADOWFEN CREATURES

### SWAMP TROLL
- **Body ID:** 53 (Troll) ✓
- **Hue:** 2212 (Swamp green)
- **BaseSoundID:** 461
- **Stats:** STR 180-220, DEX 50-70, INT 50-70
- **HP:** 180-220
- **Damage:** 8-14 (T2A Troll base)
- **Virtual Armor:** 35
- **Special:** Poison attack, Regeneration
- **Loot:** LootPack.Average
- **Taming:** Not tameable

### BOG WITCH
- **Body ID:** 87 (Evil Mage Female) ✓
- **Hue:** 2213 (Bog brown)
- **BaseSoundID:** 0x4B0
- **Stats:** STR 80-100, DEX 80-100, INT 200-250
- **HP:** 150-200
- **Damage:** 8-12 (T2A Evil Mage base)
- **Virtual Armor:** 25
- **Special:** Poison spells, Curse
- **Loot:** LootPack.Rich, LootPack.MedScrolls (2)
- **Taming:** Not tameable

### MISTWALKER
- **Body ID:** 26 (Wraith) ✓
- **Hue:** 2214 (Fog grey)
- **BaseSoundID:** 0x482
- **Stats:** STR 100-130, DEX 100-120, INT 100-150
- **HP:** 100-130
- **Damage:** 7-11 (T2A Wraith base)
- **Virtual Armor:** 28
- **Special:** Ethereal, Confusion
- **Loot:** LootPack.Meager
- **Taming:** Not tameable

### BOG BEAST
- **Body ID:** 1 (Ogre) ✓
- **Hue:** 2212 (Swamp green)
- **BaseSoundID:** 427
- **Stats:** STR 200-250, DEX 50-70, INT 30-50
- **HP:** 180-220
- **Damage:** 12-18 (T2A Ogre base)
- **Virtual Armor:** 35
- **Special:** Swamp fever, Stench
- **Loot:** LootPack.Average
- **Taming:** Not tameable

### SHADOW WOLF
- **Body ID:** 23 (Wolf) ✓
- **Hue:** 1109 (Shadow black)
- **BaseSoundID:** 0xE5
- **Stats:** STR 100-150, DEX 90-120, INT 50-75
- **HP:** 75-100
- **Damage:** 11-17 (T2A Dire Wolf base)
- **Virtual Armor:** 30
- **Special:** Stealth, Pack tactics
- **Loot:** LootPack.Average
- **Taming:** Yes - MinTame 83.1, ControlSlots 1

---

# VERDANTPEAK CREATURES

### TREANT
- **Body ID:** 47 (Reaper) ✓
- **Hue:** 2010 (Forest green)
- **BaseSoundID:** 442
- **Stats:** STR 300-400, DEX 30-50, INT 100-150
- **HP:** 300-400
- **Damage:** 12-18 (T2A Reaper base)
- **Virtual Armor:** 45
- **Special:** Poison immunity, Root attack
- **Loot:** LootPack.Rich, LootPack.Gems
- **Taming:** Not tameable

### DIRE WOLF
- **Body ID:** 23 (Wolf) ✓
- **Hue:** 2011 (Dark forest)
- **BaseSoundID:** 0xE5
- **Stats:** STR 120-150, DEX 80-100, INT 40-60
- **HP:** 120-150
- **Damage:** 11-17 (T2A Dire Wolf base)
- **Virtual Armor:** 32
- **Special:** Pack leader, Howl
- **Loot:** LootPack.Average
- **Taming:** Yes - MinTame 90.0, ControlSlots 1

### FOREST SPRITE
- **Body ID:** 58 (Wisp) ✓
- **Hue:** 2012 (Leaf green)
- **BaseSoundID:** 466
- **Stats:** STR 40-60, DEX 120-150, INT 100-150
- **HP:** 40-60
- **Damage:** 5-10 (T2A Wisp base)
- **Virtual Armor:** 20
- **Special:** Nature magic, Healing aura
- **Loot:** LootPack.Meager
- **Taming:** Not tameable

### GIANT SPIDER
- **Body ID:** 20 (Giant Spider) ✓
- **Hue:** 2010 (Forest green)
- **BaseSoundID:** 0x388
- **Stats:** STR 100-150, DEX 80-100, INT 30-50
- **HP:** 80-120
- **Damage:** 5-13 (T2A Giant Spider base)
- **Virtual Armor:** 25
- **Special:** Web, Poison
- **Loot:** LootPack.Poor
- **Taming:** Yes - MinTame 70.0, ControlSlots 1

### SHADOW DRYAD
- **Body ID:** 87 (Evil Mage Female) ✓
- **Hue:** 2213 (Dark green)
- **BaseSoundID:** 0x4B0
- **Stats:** STR 80-100, DEX 100-120, INT 150-200
- **HP:** 100-150
- **Damage:** 8-12 (T2A Evil Mage base)
- **Virtual Armor:** 25
- **Special:** Nature curse, Thorn volley
- **Loot:** LootPack.Rich, LootPack.MedScrolls (2)
- **Taming:** Not tameable

### CORRUPTED TREANT
- **Body ID:** 47 (Reaper) ✓
- **Hue:** 1109 (Dead wood)
- **BaseSoundID:** 442
- **Stats:** STR 250-350, DEX 30-50, INT 80-120
- **HP:** 250-350
- **Damage:** 12-18 (T2A Reaper base)
- **Virtual Armor:** 40
- **Special:** Disease, Rot touch
- **Loot:** LootPack.Rich, LootPack.Gems
- **Taming:** Not tameable

### THORN BEAST
- **Body ID:** 8 (Corpser) ✓
- **Hue:** 2010 (Forest green)
- **BaseSoundID:** 684
- **Stats:** STR 150-200, DEX 20-40, INT 20-40
- **HP:** 150-200
- **Damage:** 12-18 (T2A Corpser base)
- **Virtual Armor:** 35
- **Special:** Thorn volley, Root trap
- **Loot:** LootPack.Average
- **Taming:** Not tameable

---

# CRYSTAL BARRENS CREATURES

### CRYSTAL DRAKE
- **Body ID:** 60 (Drake) ✓
- **Hue:** 1154 (Crystal blue)
- **BaseSoundID:** 362
- **Stats:** STR 400-500, DEX 100-150, INT 200-300
- **HP:** 250-300
- **Damage:** 16-22 (T2A Drake base)
- **Virtual Armor:** 40
- **Special:** Energy damage, Reflection
- **Loot:** LootPack.FilthyRich, LootPack.Gems (8)
- **Taming:** Not tameable

### GEMSTONE GOLEM
- **Body ID:** 14 (Earth Elemental) ✓
- **Hue:** 1155 (Prismatic)
- **BaseSoundID:** 268
- **Stats:** STR 200-250, DEX 40-60, INT 100-150
- **HP:** 200-250
- **Damage:** 9-16 (T2A Earth Elemental base)
- **Virtual Armor:** 45
- **Special:** Spell reflection, Hardened
- **Loot:** LootPack.Average, LootPack.Meager, LootPack.Gems
- **Taming:** Not tameable

### LIGHT ELEMENTAL
- **Body ID:** 58 (Wisp) ✓
- **Hue:** 1156 (Bright white)
- **BaseSoundID:** 466
- **Stats:** STR 100-150, DEX 150-200, INT 150-200
- **HP:** 120-150
- **Damage:** 5-10 (T2A Wisp base)
- **Virtual Armor:** 30
- **Special:** Energy burst, Blind
- **Loot:** LootPack.Meager
- **Taming:** Not tameable

### CRYSTAL SPIDER
- **Body ID:** 20 (Giant Spider) ✓
- **Hue:** 1154 (Crystal blue)
- **BaseSoundID:** 0x388
- **Stats:** STR 150-200, DEX 100-150, INT 50-75
- **HP:** 100-150
- **Damage:** 5-13 (T2A Giant Spider base)
- **Virtual Armor:** 35
- **Special:** Energy poison, Web
- **Loot:** LootPack.Poor
- **Taming:** Not tameable

---

# DEEPFORGE CREATURES

### CAVE TROLL
- **Body ID:** 53 (Troll) ✓
- **Hue:** 1109 (Stone grey)
- **BaseSoundID:** 461
- **Stats:** STR 200-250, DEX 50-70, INT 40-60
- **HP:** 200-250
- **Damage:** 8-14 (T2A Troll base)
- **Virtual Armor:** 40
- **Special:** Stone skin, Darkvision
- **Loot:** LootPack.Average
- **Taming:** Not tameable

### STONE GIANT
- **Body ID:** 76 (Titan) ✓
- **Hue:** 1110 (Granite)
- **BaseSoundID:** 609
- **Stats:** STR 400-500, DEX 40-60, INT 60-80
- **HP:** 400-500
- **Damage:** 16-22 (T2A Titan base)
- **Virtual Armor:** 50
- **Special:** Boulder throw, Earthquake
- **Loot:** LootPack.FilthyRich, LootPack.Gems (8)
- **Taming:** Not tameable

### FIRE ELEMENTAL
- **Body ID:** 15 (Fire Elemental) ✓
- **Hue:** 0 (Default fire)
- **BaseSoundID:** 838
- **Stats:** STR 126-155, DEX 166-185, INT 171-192
- **HP:** 100-150
- **Damage:** 10-15 (T2A Fire Elemental base)
- **Virtual Armor:** 30
- **Special:** Fire damage 100%, Burn
- **Loot:** LootPack.Average, LootPack.Meager, LootPack.Gems
- **Taming:** Not tameable

### SALAMANDER
- **Body ID:** 35 (Lizardman) ✓
- **Hue:** 1161 (Fire red)
- **BaseSoundID:** 417
- **Stats:** STR 150-200, DEX 80-100, INT 50-70
- **HP:** 120-150
- **Damage:** 8-14 (T2A Lizardman base)
- **Virtual Armor:** 30
- **Special:** Fire immunity, Heat aura
- **Loot:** LootPack.Average
- **Taming:** Not tameable

---

# IRONCLAD EMPIRE CREATURES

### IRON GOLEM
- **Body ID:** 14 (Earth Elemental) ✓
- **Hue:** 2406 (Iron grey)
- **BaseSoundID:** 268
- **Stats:** STR 251-350, DEX 76-100, INT 101-150
- **HP:** 250-350
- **Damage:** 9-16 (T2A Earth Elemental base)
- **Virtual Armor:** 35
- **Special:** Bleed immune, Poison immune
- **Loot:** LootPack.Average, LootPack.Meager, LootPack.Gems
- **Taming:** Not tameable

### CLOCKWORK BEAST
- **Body ID:** 39 (Lizardman as mechanical) ✓
- **Hue:** 2407 (Copper)
- **BaseSoundID:** 417
- **Stats:** STR 150-200, DEX 100-120, INT 50-70
- **HP:** 150-200
- **Damage:** 12-18 (T2A Lizardman base)
- **Virtual Armor:** 30
- **Special:** Self-repair, Steam attack
- **Loot:** LootPack.Average
- **Taming:** Not tameable

### STEAMWORK GOLEM
- **Body ID:** 14 (Earth Elemental) ✓
- **Hue:** 2401 (Bronze)
- **BaseSoundID:** 268
- **Stats:** STR 300-400, DEX 40-60, INT 50-75
- **HP:** 250-350
- **Damage:** 9-16 (T2A Earth Elemental base)
- **Virtual Armor:** 45
- **Special:** Steam cloud, Repair protocol
- **Loot:** LootPack.Average, LootPack.Meager, LootPack.Gems
- **Taming:** Not tameable

### MECHANICAL SPIDER
- **Body ID:** 20 (Giant Spider) ✓
- **Hue:** 2406 (Iron grey)
- **BaseSoundID:** 0x388
- **Stats:** STR 100-150, DEX 120-150, INT 30-50
- **HP:** 100-150
- **Damage:** 5-13 (T2A Giant Spider base)
- **Virtual Armor:** 30
- **Special:** Metal web, Oil spray
- **Loot:** LootPack.Poor
- **Taming:** Not tameable

### STEAM ELEMENTAL
- **Body ID:** 13 (Air Elemental) ✓
- **Hue:** 2401 (Bronze steam)
- **BaseSoundID:** 655
- **Stats:** STR 150-200, DEX 100-150, INT 80-120
- **HP:** 150-200
- **Damage:** 10-15 (T2A Air Elemental base)
- **Virtual Armor:** 30
- **Special:** Steam cloud, Burn
- **Loot:** LootPack.Average, LootPack.Meager, LootPack.Gems
- **Taming:** Not tameable

---

# SKY/MOUNTAIN CREATURES

### GRIFFIN
- **Body ID:** 5 (Eagle) ✓
- **Hue:** 0 (Natural)
- **BaseSoundID:** 0x8F
- **Stats:** STR 200-250, DEX 120-150, INT 80-100
- **HP:** 150-200
- **Damage:** 12-18 (T2A Griffin base)
- **Virtual Armor:** 30
- **Special:** Flight, Dive attack
- **Loot:** LootPack.Rich, LootPack.Gems
- **Taming:** Yes - MinTame 95.0, ControlSlots 2

### HARPY
- **Body ID:** 30 (Harpy) ✓
- **Hue:** 0 (Natural)
- **BaseSoundID:** 402
- **Stats:** STR 100-150, DEX 100-130, INT 60-80
- **HP:** 80-120
- **Damage:** 5-7 (T2A Harpy base)
- **Virtual Armor:** 25
- **Special:** Flight, Screech
- **Loot:** LootPack.Meager (2)
- **Taming:** Not tameable

### AIR ELEMENTAL
- **Body ID:** 13 (Air Elemental) ✓
- **Hue:** 0 (Natural)
- **BaseSoundID:** 655
- **Stats:** STR 126-155, DEX 166-185, INT 101-132
- **HP:** 100-150
- **Damage:** 10-15 (T2A Air Elemental base)
- **Virtual Armor:** 28
- **Special:** Lightning, Whirlwind
- **Loot:** LootPack.Average, LootPack.Meager, LootPack.Gems
- **Taming:** Not tameable

### GARGOYLE
- **Body ID:** 4 (Gargoyle) ✓
- **Hue:** 0 (Natural)
- **BaseSoundID:** 372
- **Stats:** STR 146-175, DEX 76-95, INT 81-105
- **HP:** 100-150
- **Damage:** 10-15 (T2A Gargoyle base)
- **Virtual Armor:** 30
- **Special:** Stone form, Flight
- **Loot:** LootPack.Average
- **Taming:** Not tameable

### WIND DANCER
- **Body ID:** 30 (Harpy) ✓
- **Hue:** 1001 (Light grey)
- **BaseSoundID:** 402
- **Stats:** STR 100-150, DEX 150-200, INT 100-150
- **HP:** 80-120
- **Damage:** 5-7 (T2A Harpy base)
- **Virtual Armor:** 30
- **Special:** Wind magic, Flight
- **Loot:** LootPack.Meager (2)
- **Taming:** Not tameable

### STORM CROW
- **Body ID:** 6 (Bird) ✓
- **Hue:** 1109 (Storm grey)
- **BaseSoundID:** 0x2EE
- **Stats:** STR 50-75, DEX 100-125, INT 30-50
- **HP:** 30-45
- **Damage:** 5-7 (T2A Bird base)
- **Virtual Armor:** 15
- **Special:** Lightning peck, Flight
- **Loot:** LootPack.Poor
- **Taming:** Yes - MinTame 35.1, ControlSlots 1

### THUNDER BIRD
- **Body ID:** 5 (Eagle) ✓
- **Hue:** 1109 (Storm grey)
- **BaseSoundID:** 0x8F
- **Stats:** STR 200-250, DEX 100-150, INT 100-150
- **HP:** 150-200
- **Damage:** 12-18 (T2A Griffin base)
- **Virtual Armor:** 35
- **Special:** Lightning strike, Storm call
- **Loot:** LootPack.Rich, LootPack.Gems
- **Taming:** Not tameable

### STORM ELEMENTAL
- **Body ID:** 13 (Air Elemental) ✓
- **Hue:** 1109 (Dark grey)
- **BaseSoundID:** 655
- **Stats:** STR 150-200, DEX 150-200, INT 100-150
- **HP:** 150-200
- **Damage:** 10-15 (T2A Air Elemental base)
- **Virtual Armor:** 35
- **Special:** Lightning bolt, Thunder
- **Loot:** LootPack.Average, LootPack.Meager, LootPack.Gems
- **Taming:** Not tameable

### LIGHTNING GOLEM
- **Body ID:** 14 (Earth Elemental for golem shape) ✓
- **Hue:** 1001 (Electric blue)
- **BaseSoundID:** 268
- **Stats:** STR 200-250, DEX 100-120, INT 80-100
- **HP:** 200-250
- **Damage:** 9-16 (T2A Earth Elemental base)
- **Virtual Armor:** 35
- **Special:** Lightning field, Shock
- **Loot:** LootPack.Average, LootPack.Meager, LootPack.Gems
- **Taming:** Not tameable

---

# UNDERWATER CREATURES

### SEA SERPENT
- **Body ID:** 150 (Sea Serpent) ✓
- **Hue:** 0 (Natural)
- **BaseSoundID:** 447
- **Stats:** STR 168-225, DEX 58-85, INT 53-95
- **HP:** 110-127
- **Damage:** 10-14 (T2A Sea Serpent base)
- **Virtual Armor:** 30
- **Special:** Aquatic, Poison
- **Loot:** LootPack.Average
- **Taming:** Not tameable

### DEEP SEA SERPENT
- **Body ID:** 150 (Sea Serpent) ✓
- **Hue:** 1367 (Deep blue)
- **BaseSoundID:** 447
- **Stats:** STR 200-250, DEX 60-80, INT 60-80
- **HP:** 150-200
- **Damage:** 10-14 (T2A Sea Serpent base)
- **Virtual Armor:** 35
- **Special:** Aquatic, Greater poison
- **Loot:** LootPack.Average
- **Taming:** Not tameable

### KRAKEN
- **Body ID:** 150 (Sea Serpent, largest water creature) ✓
- **Hue:** 1367 (Deep sea)
- **BaseSoundID:** 447
- **Stats:** STR 500-600, DEX 60-80, INT 100-150
- **HP:** 500-600
- **Damage:** 10-14 (T2A Sea Serpent base)
- **Virtual Armor:** 50
- **Special:** Tentacle attack, Ink cloud
- **Loot:** LootPack.FilthyRich (2), LootPack.Gems (8)
- **Taming:** Not tameable

### WATER ELEMENTAL
- **Body ID:** 16 (Water Elemental) ✓
- **Hue:** 0 (Natural)
- **BaseSoundID:** 278
- **Stats:** STR 126-155, DEX 66-85, INT 101-125
- **HP:** 100-150
- **Damage:** 10-15 (T2A Water Elemental base)
- **Virtual Armor:** 30
- **Special:** Water attack, Drown
- **Loot:** LootPack.Average, LootPack.Meager, LootPack.Gems
- **Taming:** Not tameable

### DEEP DWELLER
- **Body ID:** 150 (Sea Serpent) ✓
- **Hue:** 1367 (Deep sea blue)
- **BaseSoundID:** 447
- **Stats:** STR 200-250, DEX 60-80, INT 40-60
- **HP:** 150-200
- **Damage:** 10-14 (T2A Sea Serpent base)
- **Virtual Armor:** 35
- **Special:** Aquatic, Tentacles
- **Loot:** LootPack.Average
- **Taming:** Not tameable

### DROWNED DEAD
- **Body ID:** 154 (Mummy) ✓
- **Hue:** 1367 (Sea blue)
- **BaseSoundID:** 471
- **Stats:** STR 150-200, DEX 40-60, INT 40-60
- **HP:** 150-200
- **Damage:** 12-18 (T2A Mummy base)
- **Virtual Armor:** 30
- **Special:** Drowning touch, Undead
- **Loot:** LootPack.Rich, LootPack.MedScrolls
- **Taming:** Not tameable

---

# SHADOW/VOID CREATURES

### SHADOW WISP
- **Body ID:** 58 (Wisp) ✓
- **Hue:** 1109 (Shadow black)
- **BaseSoundID:** 466
- **Stats:** STR 50-80, DEX 150-200, INT 100-150
- **HP:** 50-80
- **Damage:** 5-10 (T2A Wisp base)
- **Virtual Armor:** 25
- **Special:** Ethereal, Shadow magic
- **Loot:** LootPack.Meager
- **Taming:** Not tameable

### SHADE
- **Body ID:** 26 (Wraith) ✓
- **Hue:** 1109 (Pure black)
- **BaseSoundID:** 0x482
- **Stats:** STR 100-120, DEX 100-120, INT 100-120
- **HP:** 80-100
- **Damage:** 7-11 (T2A Wraith base)
- **Virtual Armor:** 28
- **Special:** Life drain, Invisible
- **Loot:** LootPack.Meager
- **Taming:** Not tameable

### SHADOW KNIGHT
- **Body ID:** 57 (Bone Knight) ✓
- **Hue:** 1109 (Shadow black)
- **BaseSoundID:** 451
- **Stats:** STR 200-250, DEX 80-100, INT 50-70
- **HP:** 200-250
- **Damage:** 15-20 (T2A Bone Knight base)
- **Virtual Armor:** 40
- **Special:** Shadow strike, Fear
- **Loot:** LootPack.Rich, LootPack.MedScrolls
- **Taming:** Not tameable

### ANCIENT LICH
- **Body ID:** 24 (Lich) ✓
- **Hue:** 1109 (Shadow)
- **BaseSoundID:** 0x3E9
- **Stats:** STR 400-500, DEX 100-120, INT 500-600
- **HP:** 400-500
- **Damage:** 24-26 (T2A Lich base)
- **Virtual Armor:** 40
- **Special:** Master necromancer, Death magic
- **Loot:** LootPack.Rich, LootPack.MedScrolls (2)
- **Taming:** Not tameable

### SHADOW FIEND
- **Body ID:** 9 (Daemon) ✓
- **Hue:** 1109 (Pure black)
- **BaseSoundID:** 357
- **Stats:** STR 300-400, DEX 100-120, INT 200-250
- **HP:** 300-400
- **Damage:** 7-14 (T2A Daemon base)
- **Virtual Armor:** 40
- **Special:** Shadow flame, Darkness
- **Loot:** LootPack.Rich, LootPack.Average (2), LootPack.MedScrolls (2)
- **Taming:** Not tameable

### VOID WALKER
- **Body ID:** 26 (Wraith) ✓
- **Hue:** 1109 (Void black)
- **BaseSoundID:** 0x482
- **Stats:** STR 150-200, DEX 100-150, INT 150-200
- **HP:** 150-200
- **Damage:** 7-11 (T2A Wraith base)
- **Virtual Armor:** 35
- **Special:** Phase shift, Void touch
- **Loot:** LootPack.Meager
- **Taming:** Not tameable

### DARK MAGE
- **Body ID:** 124 (Evil Mage) ✓
- **Hue:** 1109 (Black robes)
- **BaseSoundID:** 0x45A
- **Stats:** STR 80-100, DEX 80-100, INT 300-400
- **HP:** 150-200
- **Damage:** 8-12 (T2A Evil Mage base)
- **Virtual Armor:** 25
- **Special:** Dark magic, Curse master
- **Loot:** LootPack.Rich, LootPack.MedScrolls (2)
- **Taming:** Not tameable

---

# DUNGEON BOSSES

### FROST FATHER (Frozen Halls Boss)
- **Body ID:** 76 (Titan) ✓
- **Hue:** 1152 (Ice blue)
- **BaseSoundID:** 609
- **Stats:** STR 600-700, DEX 60-80, INT 100-150
- **HP:** 1200-1500
- **Damage:** 16-22 (T2A Titan base)
- **Virtual Armor:** 70
- **Special:** Frost shield phases, summons Frost Wraiths, cone freeze
- **Loot:** LootPack.FilthyRich (2), LootPack.Gems (8)
- **Taming:** Not tameable

### DEEPFORGE BEHEMOTH (Deepforge Mines Boss)
- **Body ID:** 76 (Titan) ✓
- **Hue:** 1177 (Dark metallic)
- **BaseSoundID:** 609
- **Stats:** STR 700-800, DEX 50-70, INT 80-120
- **HP:** 1500-1800
- **Damage:** 16-22 (T2A Titan base)
- **Virtual Armor:** 80
- **Special:** Magma burst AoE, forge empowerment, stun stomp
- **Loot:** LootPack.FilthyRich (2), LootPack.Gems (8)
- **Taming:** Not tameable

### SPHINX OF SURYA (Pyramid Boss)
- **Body ID:** 30 (Harpy) ✓
- **Hue:** 2306 (Sandstone)
- **BaseSoundID:** 402
- **Stats:** STR 500-600, DEX 120-150, INT 300-400
- **HP:** 800-1000
- **Damage:** 5-7 (T2A Harpy base)
- **Virtual Armor:** 60
- **Special:** Riddle gump, wrong answers spawn adds, sandstorm blind
- **Loot:** LootPack.FilthyRich (2), LootPack.Gems (8)
- **Taming:** Not tameable

### VOLCANO WYRM (Emberdeep Caldera Boss)
- **Body ID:** 49 (Ancient Wyrm) ✓
- **Hue:** 1359 (Deep red)
- **BaseSoundID:** 362
- **Stats:** STR 800-900, DEX 80-100, INT 200-300
- **HP:** 1000-1200
- **Damage:** 16-22 (T2A Dragon base)
- **Virtual Armor:** 80
- **Special:** Lava pools, fire breath, periodic eruption
- **Loot:** LootPack.FilthyRich (2), LootPack.Gems (8)
- **Taming:** Not tameable

### COVEN MATRIARCH (Shadowfen Crypts Boss)
- **Body ID:** 87 (Evil Mage Female) ✓
- **Hue:** 2213 (Bog brown)
- **BaseSoundID:** 0x4B0
- **Stats:** STR 200-250, DEX 100-120, INT 400-500
- **HP:** 600-800
- **Damage:** 8-12 (T2A Evil Mage base)
- **Virtual Armor:** 50
- **Special:** Curse aura, fear pulses, mirror images
- **Loot:** LootPack.FilthyRich (2), LootPack.Gems (8)
- **Taming:** Not tameable

### ANCIENT TREANT (Verdant Depths Boss)
- **Body ID:** 47 (Reaper) ✓
- **Hue:** 2010 (Forest green)
- **BaseSoundID:** 442
- **Stats:** STR 500-600, DEX 40-60, INT 200-300
- **HP:** 1000-1200
- **Damage:** 12-18 (T2A Reaper base)
- **Virtual Armor:** 70
- **Special:** Root snare, barkskin damage reduction, call of the wild adds
- **Loot:** LootPack.FilthyRich (2), LootPack.Gems (8)
- **Taming:** Not tameable

### CRYSTAL DRAKE ALPHA (Crystal Caverns Boss)
- **Body ID:** 60 (Drake) ✓
- **Hue:** 1154 (Crystal blue)
- **BaseSoundID:** 362
- **Stats:** STR 600-700, DEX 120-150, INT 300-400
- **HP:** 800-1000
- **Damage:** 16-22 (T2A Drake base)
- **Virtual Armor:** 70
- **Special:** Prismatic beam, shard storms, reflect magic window
- **Loot:** LootPack.FilthyRich (2), LootPack.Gems (8)
- **Taming:** Not tameable

### GRIFFIN LORD (Skyreach Summit Boss)
- **Body ID:** 5 (Eagle) ✓
- **Hue:** 0 (Natural)
- **BaseSoundID:** 0x8F
- **Stats:** STR 400-500, DEX 150-200, INT 150-200
- **HP:** 700-900
- **Damage:** 12-18 (T2A Griffin base)
- **Virtual Armor:** 60
- **Special:** Wind walls, knockback gusts, aerial dive burst
- **Loot:** LootPack.FilthyRich (2), LootPack.Gems (8)
- **Taming:** Not tameable

### ANCIENT KRAKEN (Abyssal Trench Boss)
- **Body ID:** 150 (Sea Serpent) ✓
- **Hue:** 1367 (Deep sea)
- **BaseSoundID:** 447
- **Stats:** STR 800-900, DEX 80-100, INT 200-300
- **HP:** 1200-1500
- **Damage:** 10-14 (T2A Sea Serpent base)
- **Virtual Armor:** 80
- **Special:** Tentacle grabs, ink cloud blind, pressure crush
- **Loot:** LootPack.FilthyRich (2), LootPack.Gems (8)
- **Taming:** Not tameable

### TIMEWORN LICH (Twilight Labyrinth Boss)
- **Body ID:** 24 (Lich) ✓
- **Hue:** 1109 (Shadow)
- **BaseSoundID:** 0x3E9
- **Stats:** STR 600-700, DEX 120-150, INT 600-700
- **HP:** 1000-1200
- **Damage:** 24-26 (T2A Lich base)
- **Virtual Armor:** 60
- **Special:** Time slow field, shadow duplicate, life drain channel
- **Loot:** LootPack.FilthyRich (2), LootPack.Gems (8)
- **Taming:** Not tameable

---

# CORRECT UO:RENAISSANCE BODY IDS

## Safe Body IDs (confirmed in UOR)
- 1: Ogre ✓
- 4: Gargoyle ✓
- 5: Eagle ✓
- 6: Bird ✓
- 8: Corpser ✓
- 9: Daemon ✓
- 12: Dragon ✓
- 13: Air Elemental ✓
- 14: Earth Elemental ✓ (USE FOR GOLEMS)
- 15: Fire Elemental ✓
- 16: Water Elemental ✓
- 20: Giant Spider ✓
- 23: Wolf ✓
- 24: Lich ✓
- 26: Wraith ✓
- 30: Harpy ✓
- 39: Lizardman/Mongbat ✓
- 47: Reaper ✓
- 48: Scorpion ✓
- 49: Ancient Wyrm ✓
- 52: Giant Beetle ✓
- 53: Troll ✓
- 57: Bone Knight ✓
- 58: Wisp ✓
- 60: Drake ✓
- 76: Titan ✓
- 87: Evil Mage Female ✓
- 98: Hell Hound ✓
- 104: Skeletal Dragon ✓
- 124: Evil Mage ✓
- 150: Sea Serpent ✓
- 154: Mummy ✓

## SUMMARY
- **Total Creatures:** 150+
- **Dungeon Bosses:** 10 major bosses
- **All Body IDs:** Verified UO:Renaissance compatible
- **No Third Dawn bodies** (no golem 752, etc.)
- **Earth Elemental (14)** used for all golem types with different hues
- **Complete Coverage:** All regions, biomes, and dungeon encounters
- **Loot Tables:** Standard T2A LootPack.* tables (no custom Vystia items)
- **Damage Values:** Base T2A era damage stats


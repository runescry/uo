"""
Extract Deepforge (Dwarf Entry) from OSI map0.mul and place it directly at new location
"""
...
OSI_MAP = Path(r'D:\UO\UO Adventures\Client\Data Files\map0.mul')
OSI_STATICS = Path(r'D:\UO\UO Adventures\Client\Data Files\statics0.mul')
OSI_STAIDX = Path(r'D:\UO\UO Adventures\Client\Data Files\staidx0.mul')
...
# Deepforge bounds on OSI map0
SOURCE_X1, SOURCE_Y1 = 1305, 318
SOURCE_X2, SOURCE_Y2 = 1413, 429

# Target location
TARGET_CENTER_X, TARGET_CENTER_Y = 5122, 2087

# Parse command line arguments
import argparse
parser = argparse.ArgumentParser(description='Extract and place Deepforge from OSI map0')
...
print("="*80)
print("EXTRACT AND PLACE DEEPFORGE FROM OSI MAP0")
if STATICS_ONLY:
...
# Read existing statics from target and filter out those in Deepforge area
...
with open(TARGET_STAIDX, 'rb') as target_idx, open(TARGET_STATICS, 'rb') as target_sta:
    for block_id in range(MAP_WIDTH_BLOCKS * MAP_HEIGHT_BLOCKS):
        ...
        if offset != 0xFFFFFFFF and length > 0:
            ...
            # Check if this block is in the Deepforge placement area
            ...
            if target_block_x1 <= block_x <= target_block_x2 and target_block_y1 <= block_y <= target_block_y2:
                # This block is in the Deepforge area - filter out statics within bounds
                filtered_data = b''
                count = length // 7
                data_pos = 0
                for _ in range(count):
                    ...
                    # Keep static only if outside Deepforge bounds
                ...
                # Block is outside Deepforge area - keep all statics
                existing_statics[block_id] = block_data
        else:
            existing_statics[block_id] = b''

# Add new Deepforge statics
print("  Adding Deepforge statics...")
for block_id, new_items in target_blocks.items():
    ...
    # Append to existing (which has been filtered to exclude Deepforge area)
...
print("\n" + "="*80)
print("DEEPFORGE PLACED SUCCESSFULLY")
print("="*80)
print(f"Source: OSI map0 ({SOURCE_X1}, {SOURCE_Y1}) to ({SOURCE_X2}, {SOURCE_Y2})")
print(f"Target: Center ({TARGET_CENTER_X}, {TARGET_CENTER_Y})")

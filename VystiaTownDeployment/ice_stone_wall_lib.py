...
WALL_STRUCTURE_SANDSTONE = {
    ...
    'foundation': 0x015E,      # Sandstone corner piece for foundation
    'wall_body': 0x0177,       # Sandstone wall body (will be rotated based on orientation)
    'wall_body_ns': 0x0176,    # Sandstone wall for east/west walls (north-south orientation)
    'wall_body_ew': 0x0177,    # Sandstone wall for north/south walls (east-west orientation)
    'floor': 0x04AB,           # Walkway floor (same)
    'hue': 0x03E9,             # Sandstone hue (1001 decimal)
    'battlements_ew': {        # East-West battlements (north/south walls)
        'merlons': 0x019A,     # Sandstone battlement for east-west
        'caps': 0x0519,        # Sandstone cap
    },
    'battlements_ns': {        # North-South battlements (east/west walls)
        'merlons': 0x0199,     # Sandstone battlement for north-south
        'caps': 0x0519,        # Sandstone cap
    }
}

# Default to sandstone for ice walls
WALL_STRUCTURE = WALL_STRUCTURE_SANDSTONE
...
def generate_advanced_wall_stack(x: int, y: int, z_base: int, is_east_west: bool, position_index: int, hue: int = 2500) -> List[Dict]:
    ...
    # Select wall body tile based on orientation (if directional tiles available)
    if 'wall_body_ew' in WALL_STRUCTURE and 'wall_body_ns' in WALL_STRUCTURE:
        wall_body_tile = WALL_STRUCTURE['wall_body_ew'] if is_east_west else WALL_STRUCTURE['wall_body_ns']
    else:
        wall_body_tile = WALL_STRUCTURE['wall_body']

    # Layer 1: Foundation (Z=0) - use directional wall tile
    items.append({
        'tile_id': wall_body_tile,
        'x': x,
        'y': y,
        'z': z_base,
        'hue': hue
    ...
    # Layer 2: Wall body (Z=20) - double stacked for chunky ice walls
    items.append({
        'tile_id': wall_body_tile,
        'x': x,
        'y': y,
...
def generate_advanced_wall_perimeter(
    ...
    for x in range(min_x, max_x + 1):
        if (x, min_y) not in gate_set:
            walls.extend(generate_advanced_wall_stack(x, min_y, z_level, True, position_index, inner_wall_hue))
            position_index += 1
    ...
    for x in range(min_x, max_x + 1):
        if (x, max_y) not in gate_set:
            walls.extend(generate_advanced_wall_stack(x, max_y, z_level, True, position_index, inner_wall_hue))
            position_index += 1
    ...
    for y in range(min_y + 1, max_y):
        if (min_x, y) not in gate_set:
            walls.extend(generate_advanced_wall_stack(min_x, y, z_level, False, position_index, inner_wall_hue))
            position_index += 1
    ...
    for y in range(min_y + 1, max_y):
        if (max_x, y) not in gate_set:
            walls.extend(generate_advanced_wall_stack(max_x, y, z_level, False, position_index, inner_wall_hue))
            position_index += 1
    ...
    # inner_wall_hue already defined from WALL_STRUCT['hue']

    # Top inner wall (south edge of top walkway) - horizontal (east-west)
    position_index = 0
    inner_wall_tile_ew = WALL_STRUCT.get('wall_body_ew', WALL_STRUCT['wall_body'])
    inner_wall_tile_ns = WALL_STRUCT.get('wall_body_ns', WALL_STRUCT['wall_body'])
    for x in range(min_x, max_x + 1):
        # Foundation at Z=0 - use directional tile
        walls.append({
            'tile_id': inner_wall_tile_ew,
            'x': x,
            'y': min_y + walkway_width + 1,
            'z': z_level,
            'hue': inner_wall_hue
        })
        # Wall body at Z=20 - double stacked
        walls.append({
            'tile_id': inner_wall_tile_ew,
            'x': x,
            'y': min_y + walkway_width + 1,
    ...
    # Bottom inner wall (north edge of bottom walkway) - horizontal (east-west)
    position_index = 0
    for x in range(min_x, max_x + 1):
        # Foundation at Z=0 - use directional tile
        walls.append({
    ...
    # Left inner wall (east edge of left walkway) - vertical (north-south)
    position_index = 0
    for y in range(min_y + walkway_width + 1, max_y - walkway_width):
        # Foundation at Z=0 - use directional tile
        walls.append({
            'tile_id': inner_wall_tile_ns,
    ...
    # Right inner wall (west edge of right walkway) - vertical (north-south)
    position_index = 0
    for y in range(min_y + walkway_width + 1, max_y - walkway_width):
        # Foundation at Z=0 - use directional tile
        walls.append({
...
def is_tree_or_vegetation(tile_id: int) -> bool:
    """
    Check if a tile ID represents a tree or vegetation that should be removed.

    Common UO tree/vegetation tile ranges:
    - 0x0C99-0x0CE8: Trees (various types)
    - 0x0CCA-0x0CD8: Tree stumps and logs
    - 0x0D41-0x0D44: Small trees
    - 0x0D45-0x0D48: Tropical trees
    - 0x0D4E-0x0D53: More trees
    - 0x0D58-0x0D70: Various trees
    ...
    for min_id, max_id in tree_ranges:
        if min_id <= tile_id <= max_id:
            return True
    return False


def filter_trees_from_area(blocks: Dict[int, List[Dict]], center_x: int, center_y: int,
                           width: int, height: int, buffer: int = 5) -> Dict[int, List[Dict]]:
    """
    Remove trees and vegetation from an area.

    Args:
        blocks: Dictionary of block_id to statics
        center_x: X coordinate of center
        center_y: Y coordinate of center
        width: Width of area
        height: Height of area
        buffer: Extra tiles outside the wall to clear (default: 5)

    Returns:
        Filtered blocks dictionary
    """
    half_width = width // 2
    half_height = height // 2

    # Calculate clearing area (wall bounds + buffer)
    min_x = center_x - half_width - buffer
    max_x = center_x + half_width + buffer
    min_y = center_y - half_height - buffer
    max_y = center_y + half_height + buffer
    ...
    for block_id, statics in blocks.items():
        ...
        for static in statics:
            ...
            abs_x = block_x * 8 + static['x']
            abs_y = block_y * 8 + static['y']

            # Check if this static is in the clearing area
            in_clearing_area = (min_x <= abs_x <= max_x and min_y <= abs_y <= max_y)

            # If it's a tree in the clearing area, skip it
            if in_clearing_area and is_tree_or_vegetation(static['tile_id']):
                trees_removed += 1
                continue

            filtered_statics.append(static)

        if filtered_statics:
            filtered_blocks[block_id] = filtered_statics

    if trees_removed > 0:
        print(f"Removed {trees_removed} trees/vegetation from area")

    ...
    public class Stealth
    {
        ...
        public static TimeSpan OnUse(Mobile m)
        {
            ...
            else
            {
                ...
                {
                    // Vystia: Religion stealth bonus (Oceana's Covenant +5 at Devoted)
                    double stealthBonus = 0.0;
                    if (m is PlayerMobile pm)
                    {
                        stealthBonus = Server.Custom.VystiaClasses.Systems.VystiaSkillIntegration.GetReligionStealthBonus(pm);
                    }
                    
                    if (m.CheckSkill(SkillName.Stealth, -20.0 + (armorRating * 2) - stealthBonus, (Core.AOS ? 60.0 : 80.0) + (armorRating * 2) + stealthBonus))
                    {

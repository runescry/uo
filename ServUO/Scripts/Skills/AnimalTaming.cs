...
namespace Server.SkillHandlers
{
	public class AnimalTaming
	{
		...
		/// <summary>
		/// Calculates anger chance for CanAngerOnTame creatures.
		/// Vystia class skills reduce anger chance:
		/// - BeastBonding: -0.7% per point (Beastmaster specialty - 0% anger at GM)
		/// - Druidism: -0.3% per point (Druid nature affinity)
		/// - Songweaving: -0.3% per point if creature is pacified (Bard synergy)
		/// Base 70% anger, minimum 5%.
		/// </summary>
		public static double GetAngerChance(Mobile tamer, BaseCreature creature)
		{
			double angerChance = 0.70; // Base 70% anger rate

			// BeastBonding - Primary taming skill (Beastmaster)
			// At 100 skill: -70% anger = 0% (never angers)
			double beastBonding = tamer.Skills[SkillName.BeastBonding].Value;
			angerChance -= beastBonding * 0.007;

			// Druidism - Nature affinity (Druid)
			// At 100 skill: -30% anger
			double druidism = tamer.Skills[SkillName.Druidism].Value;
			angerChance -= druidism * 0.003;

			// Songweaving - Only applies if creature is pacified (Bard synergy)
			// At 100 skill: -30% anger when pacified
			if (creature.BardPacified)
			{
				double bardicLore = tamer.Skills[SkillName.Songweaving].Value;
				angerChance -= bardicLore * 0.003;
			}

			// Clamp between 5% and 70%
			return Math.Max(0.05, Math.Min(0.70, angerChance));
		}
				...
				if (targeted is Mobile)
				{
					if (targeted is BaseCreature)
					{
						...
						else if (DarkWolfFamiliar.CheckMastery(from, creature) || from.Skills[SkillName.AnimalTaming].Value >= creature.CurrentTameSkill)
						{
							...
							else if (creature.CanAngerOnTame && GetAngerChance(from, creature) >= Utility.RandomDouble())
							{
			...
			private class InternalTimer : Timer
			{
				...
				protected override void OnTick()
				{
					...
					else if (m_Count < m_MaxCount)
					{
						...
						{
							m_Tamer.CheckTargetSkill(SkillName.AnimalLore, m_Creature, 0.0, 100.0);  // Vystia: 100 is GM
						}

...
namespace Server.Commands
{
    public class CommandHandlers
    {
        public static void Initialize()
        {
            ...
            // Vystia: ensure Songweaving commands are registered
            Server.Commands.SongweavingCommands.Initialize();
        }
            ...
            if (e.Length == 1)
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        // First, try to find a map by name
                        for (int i = 0; i < Map.AllMaps.Count; ++i)
                        {
                            ...
                            {
                                // Move to center of map if no specific location
                                Point3D loc = new Point3D(map.Width / 2, map.Height / 2, 0);
                                int z = map.GetAverageZ(loc.X, loc.Y);
                                loc = new Point3D(loc.X, loc.Y, z);
                                from.MoveToWorld(loc, map);
                                from.SendMessage("Teleported to {0} at {1}, {2}, {3}", map.Name, loc.X, loc.Y, loc.Z);
                                return;
                            }
                        }

                        // If map not found, try to find a region
                        ...
                        {
                            from.SendMessage("No map or region with that name was found.");
                            from.SendMessage("Available maps: {0}", String.Join(", ", Map.AllMaps.Where(m => m != null && m != Map.Internal && m.MapIndex != 0x7F && m.MapIndex != 0xFF).Select(m => m.Name)));
                        }

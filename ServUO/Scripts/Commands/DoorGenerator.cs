...
namespace Server
{
    public class DoorGenerator
    {
        ...
        // Custom map regions - Add your custom maps here
        private static readonly Dictionary<string, Rectangle2D[]> m_CustomMapRegions = new Dictionary<string, Rectangle2D[]>
        {
            // Vystia Map 9 - Full map scan (7168x4096)
            { "Vystia", new Rectangle2D[] 
                { 
                    new Rectangle2D(new Point2D(0, 0), new Point2D(7168, 4096)) // Full map scan
                    // To scan specific regions only, replace above with:
                    // new Rectangle2D(new Point2D(1000, 1000), new Point2D(2000, 2000)),
                    // new Rectangle2D(new Point2D(3000, 3000), new Point2D(4000, 4000))
                }
            },
        };
        ...
        public static void Initialize()
        {
            ...
            CommandSystem.Register("DoorGenMap", AccessLevel.Administrator, new CommandEventHandler(DoorGenMap_OnCommand));
        }

        [Usage("DoorGen")]
        [Description("Generates doors by analyzing the map. Processes all OSI maps (Trammel, Felucca, Ilshenar, Malas). Slow.")]
        public static void DoorGen_OnCommand(CommandEventArgs e)
        {
            Generate();
        }

        [Usage("DoorGenMap <mapname>")]
        [Description("Generates doors for a specific map. Use map name (e.g., 'Vystia', 'Trammel', 'Felucca') or 'all' for all maps including custom maps.")]
        public static void DoorGenMap_OnCommand(CommandEventArgs e)
        {
            if (e.Length < 1)
            {
                e.Mobile.SendMessage("Usage: [DoorGenMap <mapname>");
                e.Mobile.SendMessage("Examples: [DoorGenMap Vystia");
                e.Mobile.SendMessage("         [DoorGenMap Trammel");
                e.Mobile.SendMessage("         [DoorGenMap all");
                e.Mobile.SendMessage("Available maps: Trammel, Felucca, Ilshenar, Malas, Tokuno, TerMur");
                e.Mobile.SendMessage("Plus any custom maps defined in DoorGenerator.cs");
                return;
            }

            string mapName = e.GetString(0);

            if (mapName.ToLower() == "all")
            {
                GenerateAllMaps();
            }
            else
            {
                GenerateForMap(mapName);
            }
        }
        ...
        /// <summary>
        /// Generates doors for all maps including custom maps
        /// </summary>
        public static void GenerateAllMaps()
        {
            World.Broadcast(0x35, true, "Generating doors for all maps, please wait.");
            ...
            // Process OSI maps
            int trammelCount = GenerateForMap("Trammel");
            if (trammelCount >= 0)
            {
                totalCount += trammelCount;
                results.Add(string.Format("Trammel: {0}", trammelCount));
            }

            int feluccaCount = GenerateForMap("Felucca");
            if (feluccaCount >= 0)
            {
                totalCount += feluccaCount;
                results.Add(string.Format("Felucca: {0}", feluccaCount));
            }

            int ilshenarCount = GenerateForMap("Ilshenar");
            if (ilshenarCount >= 0)
            {
                totalCount += ilshenarCount;
                results.Add(string.Format("Ilshenar: {0}", ilshenarCount));
            }

            int malasCount = GenerateForMap("Malas");
            if (malasCount >= 0)
            {
                totalCount += malasCount;
                results.Add(string.Format("Malas: {0}", malasCount));
            }

            // Process custom maps
            foreach (var kvp in m_CustomMapRegions)
            {
                int customCount = GenerateForMap(kvp.Key);
                if (customCount >= 0)
                {
                    totalCount += customCount;
                    results.Add(string.Format("{0}: {1}", kvp.Key, customCount));
                }
            }
            ...
            World.Broadcast(0x35, true, "Door generation complete. Total: {0}. {1}", totalCount, resultMessage);
        }

        /// <summary>
        /// Generates doors for a specific map by name
        /// </summary>
        /// <param name="mapName">Name of the map (e.g., "Trammel", "Vystia")</param>
        /// <returns>Number of doors generated, or -1 if map not found</returns>
        public static int GenerateForMap(string mapName)
        {
            Map targetMap = null;
            Rectangle2D[] regions = null;

            // Try to find map by name
            foreach (Map map in Map.AllMaps)
            {
                if (map != null && map.Name != null && map.Name.Equals(mapName, StringComparison.OrdinalIgnoreCase))
                {
                    targetMap = map;
                    break;
                }
            ...
            if (targetMap == null)
            {
                World.Broadcast(0x35, true, "Map '{0}' not found.", mapName);
                return -1;
            }

            // Get regions for this map
            if (mapName.Equals("Trammel", StringComparison.OrdinalIgnoreCase) || 
                mapName.Equals("Felucca", StringComparison.OrdinalIgnoreCase))
            {
                regions = m_BritRegions;
            }
            else if (mapName.Equals("Ilshenar", StringComparison.OrdinalIgnoreCase))
            {
                regions = m_IlshRegions;
            }
            else if (mapName.Equals("Malas", StringComparison.OrdinalIgnoreCase))
            {
                regions = m_MalasRegions;
            }
            else if (m_CustomMapRegions.ContainsKey(mapName))
            {
                regions = m_CustomMapRegions[mapName];
            }
            else
            {
                // Default: scan entire map
                regions = new Rectangle2D[]
                {
                    new Rectangle2D(new Point2D(0, 0), new Point2D(targetMap.Width, targetMap.Height))
                };
            }

            World.Broadcast(0x35, true, "Generating doors for {0}...", mapName);
            ...
            World.Broadcast(0x35, true, "Generated {0} doors for {1}.", m_Count, mapName);
            return m_Count;
        }
        ...
        public static BaseDoor AddDoor(int x, int y, int z, DoorFacing facing)
        {
            ...
            // OSI-specific exclusion zones (only apply to Britannia maps)
            // These are hardcoded coordinates from original OSI maps
            // You can remove or modify these for custom maps
            if (m_Map == Map.Trammel || m_Map == Map.Felucca)
            {

...
// Spawner baseline configuration is documented in SpawnerBaseline.cs
// This defines the default attributes for spawners created through world generation scripts.

namespace Server.Commands 
{
	/// <summary>
	/// Manages world creation, deletion, and recreation commands.
	/// Spawner attributes used in world generation follow the baseline configuration
	/// defined in SpawnerBaseline.cs, which documents all attributes currently
	/// set via decoration files (Decorate.cs).
	/// </summary>
	public class CreateWorld
	{
        ...
        { 
            ...
            CommandSystem.Register("UpdateSpawnerBaseline", AccessLevel.Administrator, new CommandEventHandler(UpdateBaseline_OnCommand));
            CommandSystem.Register("ScanWorldState", AccessLevel.Administrator, new CommandEventHandler(ScanWorldState_OnCommand));
		}
        ...
        /// <summary>
        /// Applies baseline spawner configuration to a spawner instance.
        /// This ensures spawners created through world generation use consistent baseline values.
        /// See SpawnerBaseline.cs for documented default values.
        /// </summary>
        /// <param name="spawner">The spawner to configure with baseline values</param>
        public static void ApplySpawnerBaseline(Spawner spawner)
        {
            SpawnerBaseline.ApplyBaseline(spawner);
        }

        /// <summary>
        /// Gets a summary of the current spawner baseline configuration.
        /// Useful for documentation and debugging purposes.
        /// </summary>
        /// <returns>Formatted string containing all baseline attribute values</returns>
        public static string GetSpawnerBaselineSummary()
        {
            return SpawnerBaseline.GetBaselineSummary();
        }

        [Usage("UpdateSpawnerBaseline [mode|median|mean] [apply] [code]")]
        [Description("Scans current world state to calculate baseline spawner values. Use 'apply' to update defaults, 'code' to generate code.")]
        private static void UpdateBaseline_OnCommand(CommandEventArgs e)
        {
            SpawnerBaseline.CalculationMode mode = SpawnerBaseline.CalculationMode.Mode;
            bool apply = false;
            bool generateCode = false;

            if (e.Arguments.Length > 0)
            {
                string modeStr = e.Arguments[0].ToLower();
                if (modeStr == "median")
                    mode = SpawnerBaseline.CalculationMode.Median;
                else if (modeStr == "mean")
                    mode = SpawnerBaseline.CalculationMode.Mean;
                else if (modeStr == "apply")
                    apply = true;
                else if (modeStr == "code")
                    generateCode = true;
            }

            for (int i = 0; i < e.Arguments.Length; i++)
            {
                string arg = e.Arguments[i].ToLower();
                if (arg == "apply")
                    apply = true;
                else if (arg == "code")
                    generateCode = true;
            ...
            e.Mobile.SendMessage("Scanning world for spawners...");
            var result = SpawnerBaseline.CalculateFromWorldState(mode);

            if (!result.IsValid)
            {
                e.Mobile.SendMessage("No spawners found in the world. Baseline remains at code defaults.");
                e.Mobile.SendMessage("Current baseline:\n{0}", SpawnerBaseline.GetBaselineSummary());
                return;
            }

            e.Mobile.SendMessage("Baseline calculated from {0} spawners using {1} method:", result.TotalSpawners, mode);
            e.Mobile.SendMessage(result.ToString());

            if (generateCode)
            {
                string code = SpawnerBaseline.GenerateCodeFromResult(result);
                e.Mobile.SendMessage("=== CODE TO PASTE INTO SpawnerBaseline.cs Defaults class ===");
                e.Mobile.SendMessage(code);
                e.Mobile.SendMessage("=== END CODE ===");
                
                // Also write to file
                string filePath = "SpawnerBaseline_Generated.cs";
                System.IO.File.WriteAllText(filePath, code);
                e.Mobile.SendMessage("Code also written to: {0}", filePath);
            }

            if (apply)
            {
                SpawnerBaseline.ApplyWorldStateBaseline(result);
                e.Mobile.SendMessage("Baseline defaults have been updated from world state.");
            }
            else if (!generateCode)
            {
                e.Mobile.SendMessage("Use 'UpdateSpawnerBaseline {0} apply' to update the baseline defaults.", mode.ToString().ToLower());
                e.Mobile.SendMessage("Use 'UpdateSpawnerBaseline {0} code' to generate code for pasting into SpawnerBaseline.cs", mode.ToString().ToLower());
            }
        ...
        [Usage("ScanWorldState [code] [save]")]
        [Description("Scans complete world state including spawners and all decorated items/statics. Use 'code' to generate spawner baseline code, 'save' to save report to file.")]
        private static void ScanWorldState_OnCommand(CommandEventArgs e)
        {
            bool generateCode = false;
            ...
            e.Mobile.SendMessage("Scanning complete world state (spawners and decorated items)...");
            e.Mobile.SendMessage("This may take a moment...");

            var result = WorldStateBaseline.ScanWorldState();

            // Generate report
            string report = WorldStateBaseline.GenerateReport(result);
            e.Mobile.SendMessage(report);

            // Generate code if requested
            if (generateCode)
            {
                string code = WorldStateBaseline.GenerateSpawnerBaselineCode(result);
                e.Mobile.SendMessage("=== SPAWNER BASELINE CODE ===");
                e.Mobile.SendMessage(code);
                e.Mobile.SendMessage("=== END CODE ===");

                string codeFile = "SpawnerBaseline_Generated.cs";
                File.WriteAllText(codeFile, code);
                e.Mobile.SendMessage("Code written to: {0}", codeFile);
            }

            // Save to file if requested
            if (saveToFile)
            {
                WorldStateBaseline.SaveToFile(result);
                e.Mobile.SendMessage("Report saved to: WorldStateBaseline.txt");
            }
            else
            {
                e.Mobile.SendMessage("Use 'ScanWorldState save' to save the full report to a file.");
            }
        }

...
namespace Server.Spells.Chivalry
{
	public abstract class PaladinSpell : Spell
	{
		...
		public static int ComputePowerValue(Mobile from, int div)
		{
			...
			// Vystia: HolyDevotion spell power bonus (+0.5% per 10 skill, 50% at GM)
			if (from is PlayerMobile pm && pm.Skills != null)
			{
				double holyDevotion = pm.Skills[SkillName.HolyDevotion].Value;
				if (holyDevotion > 0)
				{
					double spellBonus = 1.0 + (holyDevotion * 0.005); // +0.5% per 10 skill
					v = (int)(v * spellBonus);
				}
			}

...
namespace Server.Spells
{
    ...
    {
        // Increased from 745 to 1500 to accommodate Vystia custom spells (IDs 999-1383)
        private static readonly Type[] m_Types = new Type[1500];
        ...
        public static Spell NewSpell(int spellID, Mobile caster, Item scroll)
        {
            if (spellID < 0 || spellID >= m_Types.Length)
            {
                Console.WriteLine($"[REGISTRY] NewSpell({spellID}) - OUT OF RANGE (max: {m_Types.Length - 1})");
                return null;
            }
            ...
            if (t == null)
            {
                // Failsafe: attempt to (re)initialize Vystia spells, then retry
                VystiaSpellInitializer.Initialize();
                t = m_Types[spellID];

                if (t == null)
                {
                    Console.WriteLine($"[REGISTRY] NewSpell({spellID}) - Type is NULL (not registered)");
                    return null;
                }
            }
            ...
            {
                Console.WriteLine($"[REGISTRY] NewSpell({spellID}) - Type {t.Name} is SpecialMove, not Spell");
                return null;
            }
            ...
            try
            {
                Spell spell = (Spell)Activator.CreateInstance(t, m_Params);
                Console.WriteLine($"[REGISTRY] NewSpell({spellID}) - SUCCESS: Created {t.Name}");
                return spell;
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[REGISTRY] NewSpell({spellID}) - EXCEPTION creating {t.Name}: {ex.Message}");
            }

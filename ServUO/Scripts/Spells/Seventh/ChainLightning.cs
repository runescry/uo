    ...
    public class ChainLightningSpell : MagerySpell
    {
        ...
        public void Target(IPoint3D p)
        {
            ...
            else if (SpellHelper.CheckTown(p, Caster) && CheckSequence())
            {
                ...
                var targets = AcquireIndirectTargets(p, 4).ToList(); // Increased from 2 to 4 tiles for better AoE
                var count = Math.Max(1, targets.Count);

                if (targets.Count == 0)
                {
                    Caster.SendMessage("The chain lightning finds no targets in range.");
                    // Still show visual effect at target location
                    Effects.SendBoltEffect(EffectMobile.Create(new Point3D(p), Caster.Map, EffectMobile.DefaultDuration), true, 0, false);
                }

    ...
    public class MeteorSwarmSpell : MagerySpell
    {
        ...
        public void Target(IPoint3D p, Item item)
        {
            ...
            else if (SpellHelper.CheckTown(p, Caster) && (item != null || CheckSequence()))
            {
                ...
                var targets = AcquireIndirectTargets(p, 5).ToList(); // Increased from 2 to 5 tiles (standard Meteor Swarm AoE)
                var count = Math.Max(1, targets.Count);

                if (targets.Count == 0)
                {
                    Caster.SendMessage("The meteor swarm finds no targets in range.");
                    Effects.PlaySound(p, Caster.Map, 0x160);
                    Effects.SendLocationParticles(EffectItem.Create(new Point3D(p), Caster.Map, EffectItem.DefaultDuration), 0x36BD, 20, 10, 0, 0, 5052, 0);
                }
                else
                {
                    Effects.PlaySound(p, Caster.Map, 0x160);

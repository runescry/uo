			...
			if ( p != null )
			{
				...
				for(int i=0; i < m_Request.Items.Count; ++i)
				{
					...
					try
					{
						...
						#region MobileSaver - Commented out: MobileSaver class was removed to avoid conflicts
						/*
						if ( item.ItemID == 0x1 )
						{
		...
		private Item CreateItem(int ItemID) // Create the appropriate item class - Defaults to a Static Item
		{
			...
			switch(ItemID)
			{
				...
				*/
				...
				// Housing Metal Doors - Commented out: These door classes were removed to avoid conflicts with ServUO
				// Use standard MetalDoor classes instead if needed
				/*
				case 0x679:
				...
				*/
				...
				// SE Doors - Commented out: These door classes were removed to avoid conflicts with ServUO
				// Use standard door classes instead if needed
				/*

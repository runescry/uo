...
namespace Server.Misc
{
	public class SkillCheck
	{
		...
		private static readonly bool[] UseAntiMacro =
		{
			...
			true, // Cryomancy = 58
			true, // Demonology = 59
			true, // NecromancyArts = 60
			true, // Druidism = 61
			true, // Elementalism = 62
			true, // Songweaving = 63
			true, // Hexcraft = 64
			true, // Divination = 65
			true, // Conjuration = 66
			true, // SpiritCalling = 67
			true, // Runeweaving = 68
			true, // IllusionMagic = 69

			// Martial Class Skills
			true, // Berserking = 70
			true, // Subterfuge = 71
			true, // MartialArts = 72
			true, // ChivalricArts = 73
			true, // HolyDevotion = 74
			true, // Marksmanship = 75
			true, // CombatMastery = 76
			true, // Zealotry = 77
			true, // Manhunting = 78
			true, // BeastBonding = 79
			true, // Engineering = 80
			true, // Transmutation = 81
			true, // DivineGrace = 82
			true  // ArcaneStudies = 83
			#endregion
		};
		...
		{
		    // Only debug Vystia skills (58-83)
		    bool isVystiaSkill = (int)skill.SkillName >= 58 && (int)skill.SkillName <= 83;
            ...
            bool allowGain = AllowGain(from, skill, obj);
            if (isVystiaSkill)
                from.SendMessage(0x35, "[VYSTIA CheckSkill] {0}({1}): AllowGain={2}, gc={3:F4}, Base={4:F1}",
                    skill.SkillName, (int)skill.SkillName, allowGain, gc, skill.Base);

			if (allowGain)
			{
				bool shouldGain = from.Alive && (skill.Base < 10.0 || Utility.RandomDouble() <= gc || CheckGGS(from, skill));
				if (isVystiaSkill)
				    from.SendMessage(0x35, "[VYSTIA CheckSkill] ShouldGain={0} (Base<10={1})", shouldGain, skill.Base < 10.0);

				if (shouldGain)
				{
				    if (isVystiaSkill)
				        from.SendMessage(0x35, "[VYSTIA CheckSkill] -> Calling Gain()");
					Gain(from, skill);
				}
			}
		...
		{
		    // Only debug Vystia skills (58-83) to reduce spam
		    bool isVystiaSkill = (int)skill.SkillName >= 58 && (int)skill.SkillName <= 83;
		    if (isVystiaSkill)
		    {
		        from.SendMessage(0x35, "[VYSTIA Gain] Enter: toGain={0}, skill={1}({2}), Base={3:F1}, Cap={4:F1}, Lock={5}",
		            toGain, skill.SkillName, (int)skill.SkillName, skill.Base, skill.Cap, skill.Lock);
		    }
			...
			{
			    if (isVystiaSkill)
			        from.SendMessage(0x35, "[VYSTIA Gain] Passed Base<Cap and Lock=Up check");
				...
				if (isVystiaSkill)
				    from.SendMessage(0x35, "[VYSTIA Gain] Skills.Total={0:F1}, Skills.Cap={1}, toGain={2}",
				        skills.Total, skills.Cap, toGain);
				...
				if (!from.Player || (skills.Total + toGain <= skills.Cap))
				{
					...
					if (isVystiaSkill)
					    from.SendMessage(0x35, "[VYSTIA Gain] SUCCESS! {0}: {1:F1} -> {2:F1}", skill.SkillName, oldBase, skill.Base);
				...
				{
				    if (isVystiaSkill)
				        from.SendMessage(0x22, "[VYSTIA Gain] BLOCKED: Total+toGain ({0}) > Cap ({1})",
			...
			{
			    if (isVystiaSkill)
			        from.SendMessage(0x22, "[VYSTIA Gain] BLOCKED: Base({0:F1}) >= Cap({1:F1}) OR Lock={2}",
			            skill.Base, skill.Cap, skill.Lock);
			}

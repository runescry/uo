        ...
        public void EndHeal()
        {
            ...
            else
            {
                ...
                if (chance > Utility.RandomDouble())
                {
					...
					if (m_Healer is PlayerMobile pm && pm.Skills != null)
					{
						double divineGrace = pm.Skills[SkillName.DivineGrace].Value;
						if (divineGrace > 0)
						{
							double healBonus = 1.0 + (divineGrace * 0.005); // +0.5% per 10 skill
							toHeal *= healBonus;
						}

						// Vystia: Lunara's Covenant Devoted: Healing +5%
						double religionHealingBonus = Server.Custom.VystiaClasses.Systems.VystiaSkillIntegration.GetReligionHealingBonus(pm);
						if (religionHealingBonus > 0)
						{
							toHeal = toHeal * (1.0 + religionHealingBonus); // +5% healing
						}

						// Vystia: Religion healing effectiveness (opposed religion: 50% effectiveness)
						if (m_Patient is PlayerMobile targetPM)
						{
							...
							toHeal = toHeal * effectiveness; // 1.0 for same/neutral, 0.5 for opposed
						}

						// Vystia: Blessed item healing bonus (Lunara's Covenant)
						double blessedHealingBonus = Server.Custom.VystiaClasses.Systems.VystiaSkillIntegration.GetBlessedItemHealingBonus(pm);
						if (blessedHealingBonus > 0)
						{
							toHeal = toHeal * (1.0 + blessedHealingBonus); // +5% or +10% healing
						}
					}
					...
					// Vystia: Generate Faith resource on successful heal
					if (m_Healer is PlayerMobile pmHealer)
					{
						var manager = Server.Custom.VystiaClasses.Systems.VystiaResourceManager.GetManager(pmHealer);
						if (manager != null)
						{
							manager.OnHeal(m_Patient, (int)toHeal, false);
						}
					}

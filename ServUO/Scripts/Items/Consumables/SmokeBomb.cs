        ...
        public override void OnDoubleClick(Mobile from)
		{
			...
			else
			{
				...
				if (from.UseSkill(SkillName.Hiding))
				{
					...
					// Auto-unhide after 12 seconds
					Timer.DelayCall(TimeSpan.FromSeconds(12.0), () =>
					{
						if (from != null && !from.Deleted && from.Hidden)
						{
							// Ensure staff are revealed too (RevealingAction only unhides players).
							from.Hidden = false;
							from.RevealingAction();
							from.SendLocalizedMessage(500814); // You have been revealed!
						}
					});
				}

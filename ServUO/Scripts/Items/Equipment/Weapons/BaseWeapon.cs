...
namespace Server.Items
{
    ...
    public abstract class BaseWeapon : Item, IWeapon, IFactionItem, IUsesRemaining, ICraftable, ISlayer, IDurability, ISetItem, IVvVItem, IOwnerRestricted, IResource, IArtifact, ICombatEquipment, IEngravable, IQuality, IBlessedItem
    {
        ...
        */
		...
		// Vystia: Blessed item properties
		private VystiaReligion m_BlessingReligion;
		private BlessingType m_BlessingType;
		private DateTime m_BlessedDate;
		...
		// Vystia: IBlessedItem implementation
		[CommandProperty(AccessLevel.GameMaster)]
		public VystiaReligion BlessingReligion
		{
			get { return m_BlessingReligion; }
			set { m_BlessingReligion = value; InvalidateProperties(); }
		}

		[CommandProperty(AccessLevel.GameMaster)]
		public BlessingType BlessingType
		{
			get { return m_BlessingType; }
			set { m_BlessingType = value; InvalidateProperties(); }
		}

		[CommandProperty(AccessLevel.GameMaster)]
		public DateTime BlessedDate
		{
			get { return m_BlessedDate; }
			set { m_BlessedDate = value; }
		}

		public override bool OnEquip(Mobile from)
		{
			// Vystia: Check if player can equip blessed item
			if (from is PlayerMobile pm && m_BlessingReligion != VystiaReligion.None)
			{
				if (!VystiaBlessedItemSystem.CanEquipBlessedItem(pm, this))
				{
					pm.SendMessage(0x22, "You cannot equip an item blessed by an opposed religion!");
					return false;
				}
			}
		...
		public virtual bool CheckHit(Mobile attacker, IDamageable damageable)
		{
			...
			if (Core.AOS)
			{
                ...
                // Vystia: Blessed item hit chance bonus (Celestis Arcanum)
                if (attacker is PlayerMobile pm)
                {
                    double hitChanceBonus = Server.Custom.VystiaClasses.Systems.VystiaSkillIntegration.GetBlessedItemHitChanceBonus(pm);
                    bonus += (int)(hitChanceBonus * 100); // Convert percentage to integer (5% = 5)
                }
		...
		public static bool CheckParry(Mobile defender)
		{
			...
			if (shield != null || !defender.Player)
			{
				...
				// Vystia: Knight + Frosthelm Faith: +5% block chance
				if (defender is PlayerMobile pm)
				{
					double blockBonus = Server.Custom.VystiaClasses.Systems.VystiaSkillIntegration.GetClassReligionSynergyBlockBonus(pm);
					chance += blockBonus; // Add +5% block chance
				}
				...
				// Vystia: CombatMastery block bonus (+0.1% per skill point)
				chance += Server.Custom.VystiaClasses.Systems.VystiaSkillIntegration.GetCombatMasteryBlockBonus(defender);
			...
			else if (!(defender.Weapon is Fists) && !(defender.Weapon is BaseRanged))
			{
				...
				// Vystia: CombatMastery block bonus (+0.1% per skill point)
				double combatMasteryBonus = Server.Custom.VystiaClasses.Systems.VystiaSkillIntegration.GetCombatMasteryBlockBonus(defender);
				chance += combatMasteryBonus;
				aosChance += combatMasteryBonus;
		...
		public virtual int AbsorbDamageAOS(Mobile attacker, Mobile defender, int damage)
		{
                                                            ...
                                                            defender.Skills[SkillName.Wrestling].Base >= 100))
			{
				...
				if (blocked)
				{
					...
					// Vystia: Generate Discipline resource on successful block
					if (defender is PlayerMobile pm)
					{
						var manager = Server.Custom.VystiaClasses.Systems.VystiaResourceManager.GetManager(pm);
						manager?.OnBlock();
					}
        ...
        public virtual void OnHit(Mobile attacker, IDamageable damageable, double damageBonus)
		{
            ...
            if (acidicTarget || (defender != null && splintering) || Utility.Random(40) <= chance)
            {
                ...
                // Vystia: Blessed item self-repair bonus (Cogsmith Creed Critical)
                if (attacker is PlayerMobile pmBlessed)
                {
                    int blessedSelfRepair = Server.Custom.VystiaClasses.Systems.VystiaSkillIntegration.GetBlessedItemSelfRepairBonus(pmBlessed, this);
                    if (blessedSelfRepair > 0)
                    {
                        selfRepair += blessedSelfRepair; // Add +2 self-repair
                    }
                }
            ...
            */
			...
			// Vystia: Focus resource crit bonus (+20% crit chance at 80+ Focus for ranged weapons)
			if (attacker is PlayerMobile pmFocus && this is BaseRanged)
			{
				var focus = Server.Custom.VystiaClasses.Systems.VystiaResourceManager.GetResource<FocusResource>(pmFocus);
				if (focus != null && focus.Current >= 80.0)
				{
					// Add 20% to crit chance via percentage bonus (crits are handled separately)
					// This increases overall damage by effectively adding crit chance
					percentageBonus += 20;
				}
			}

			// Vystia: Barbarian Deathblow execute (+100% damage at <25% HP)
			if (attacker is PlayerMobile pmBarbarian && defender != null)
			{
				double hpPercent = (double)defender.Hits / defender.HitsMax;
				if (hpPercent < 0.25) // Less than 25% HP
				{
					// Check if attacker has Berserking skill
					if (pmBarbarian.Skills != null && pmBarbarian.Skills[SkillName.Berserking].Value > 0)
					{
						percentageBonus += 100; // +100% damage
						pmBarbarian.SendMessage(0x22, "Deathblow!");
					}
				}
			}

			// Vystia: Bounty Hunter marked target bonuses
			if (attacker is PlayerMobile pmBounty && defender != null)
			{
				// Check if target has BountyMark
				var tracker = Server.Custom.VystiaClasses.Systems.VystiaTargetTracker.Instance;
				if (tracker != null && tracker.GetMarkType(pmBounty, defender) == Server.Custom.VystiaClasses.Systems.MarkType.BountyMark)
				{
					// +10% damage to marked targets
					percentageBonus += 10;

					// Execute: +50% damage at <30% HP
					double hpPercent = (double)defender.Hits / defender.HitsMax;
					if (hpPercent < 0.30) // Less than 30% HP
					{
						percentageBonus += 50; // +50% execute damage
						pmBounty.SendMessage(0x22, "Execute!");
					}
				}
			}

			// Vystia: Templar execute (+60-100 damage at <20% HP with mark and Zeal)
			if (attacker is PlayerMobile pmTemplar && defender != null)
			{
				var tracker = Server.Custom.VystiaClasses.Systems.VystiaTargetTracker.Instance;
				if (tracker != null && tracker.GetMarkType(pmTemplar, defender) == Server.Custom.VystiaClasses.Systems.MarkType.Divine)
				{
					double hpPercent = (double)defender.Hits / defender.HitsMax;
					if (hpPercent < 0.20) // Less than 20% HP
					{
						var zeal = Server.Custom.VystiaClasses.Systems.VystiaResourceManager.GetResource<ZealResource>(pmTemplar);
						if (zeal != null && zeal.Current > 0)
						{
							// Execute bonus: 60-100 damage based on Zeal (12-20 per Zeal, max 5)
							int zealStacks = Math.Min(5, zeal.Current);
							int executeDamage = 60 + (zealStacks * 8); // 60 + (0-40) = 60-100
							damage += executeDamage;
							pmTemplar.SendMessage(0x22, "Templar Execute!");
						}
					}
				}
			}
			...
			// Vystia: Chi resource generation on hit (unarmed only)
			if (attacker is PlayerMobile pmChi && this is Fists)
			{
				var chi = Server.Custom.VystiaClasses.Systems.VystiaResourceManager.GetResource<ChiResource>(pmChi);
				if (chi != null && chi.Current < chi.Maximum)
				{
					chi.Generate(1);
				}
			}

			// Vystia: Touch of Death execute (5 Chi, target <10% HP = instant kill)
			if (attacker is PlayerMobile pmChiDeath && this is Fists && defender != null)
			{
				var chi = Server.Custom.VystiaClasses.Systems.VystiaResourceManager.GetResource<ChiResource>(pmChiDeath);
				if (chi != null && chi.Current >= 5)
				{
					double hpPercent = (double)defender.Hits / defender.HitsMax;
					if (hpPercent < 0.10) // Less than 10% HP
					{
						// Instant kill - set damage to target's current HP
						damage = defender.Hits;
						chi.Spend(5); // Consume all Chi
						pmChiDeath.SendMessage(0x22, "Touch of Death!");
						defender.SendMessage(0x22, "You have been struck by Touch of Death!");
					}
				}
			}
			...
			// Vystia: Generate threat on hit for Knight class
			if (attacker is PlayerMobile pmThreat && defender is BaseCreature bcDefender)
			{
				Server.Custom.VystiaClasses.Systems.ThreatSystem.AddThreat(bcDefender, pmThreat, damage);
			}
			...
			// Vystia: Generate secondary resources from weapon damage
			if (attacker is PlayerMobile pmResourceGen && damageGiven > 0)
			{
				var manager = Server.Custom.VystiaClasses.Systems.VystiaResourceManager.GetManager(pmResourceGen);
				bool isCrit = false; // TODO: Track crits if needed
				manager?.OnDamageDealt(defender, damageGiven, isCrit);
			}
		...
		public virtual void OnMiss(Mobile attacker, IDamageable damageable)
		{
			...
			// Vystia: Focus resource consumption on miss (-10 Focus)
			if (attacker is PlayerMobile pmFocusMiss && this is BaseRanged)
			{
				var focus = Server.Custom.VystiaClasses.Systems.VystiaResourceManager.GetResource<FocusResource>(pmFocusMiss);
				if (focus != null)
				{
					focus.Spend(10);
				}
		...
		public virtual void GetBaseDamageRange(Mobile attacker, out int min, out int max)
		{
            ...
            else if (this is Fists && attacker is PlayerMobile)
            {
				// Vystia: MartialArts unarmed damage bonus (+1 per 20 skill, 5 at GM)
				int martialArtsBonus = Server.Custom.VystiaClasses.Systems.VystiaSkillIntegration.GetMartialArtsUnarmedBonus(attacker);
				min = MinDamage + martialArtsBonus;
				max = MaxDamage + martialArtsBonus;
            }
            else
            {
                min = MinDamage;
		...
		public virtual double ScaleDamageAOS(Mobile attacker, double damage, bool checkSkills)
		{
            ...
            */
			...
			#region Vystia Class Skill Bonuses
			// CombatMastery: +0.2% per 10 skill (20% at GM)
			double combatMasteryBonus = 0.0;
			if (attacker != null)
			{
				combatMasteryBonus = Server.Custom.VystiaClasses.Systems.VystiaSkillIntegration.GetCombatMasteryDamageBonus(attacker);
			}

			// Marksmanship: +0.3% per 10 skill (30% at GM) for ranged weapons only
			double marksmanshipBonus = 0.0;
			if (attacker != null && (this is BaseRanged || Type == WeaponType.Ranged))
			{
				marksmanshipBonus = Server.Custom.VystiaClasses.Systems.VystiaSkillIntegration.GetMarksmanshipDamageBonus(attacker, true);
			}

			// Berserking: Fury-based damage bonus (+15% at 50+, +30% at 80+)
			double berserkingBonus = 0.0;
			if (attacker is PlayerMobile pmAttacker)
			{
				berserkingBonus = Server.Custom.VystiaClasses.Systems.VystiaSkillIntegration.GetBerserkingDamageBonus(pmAttacker);
			}
			#endregion

			double totalBonus = strengthBonus + anatomyBonus + tacticsBonus + lumberBonus +
								((GetDamageBonus() + damageBonus) / 100.0) + combatMasteryBonus + marksmanshipBonus + berserkingBonus;

			// Vystia: Blessed item fire damage bonus (Cogsmith Creed)
			if (attacker is PlayerMobile pm)
			{
				double fireDamageBonus = Server.Custom.VystiaClasses.Systems.VystiaSkillIntegration.GetBlessedItemFireDamageBonus(pm, this);
				if (fireDamageBonus > 0.0)
				{
					totalBonus += fireDamageBonus; // +5% or +10% fire damage
				}
			}
		...
		public override void Serialize(GenericWriter writer)
		{
            ...
            SetSaveFlag(ref flags, SaveFlag.BlessedItem, m_BlessingReligion != VystiaReligion.None);
            ...
            if (GetSaveFlag(flags, SaveFlag.BlessedItem))
            {
                writer.Write((int)m_BlessingReligion);
                writer.Write((int)m_BlessingType);
                writer.Write(m_BlessedDate);
            }
		}
				...
				case 5:
					{
                        ...
                        // Vystia: Blessed item data
                        if (GetSaveFlag(flags, SaveFlag.BlessedItem))
                        {
                            m_BlessingReligion = (VystiaReligion)reader.ReadInt();
                            m_BlessingType = (BlessingType)reader.ReadInt();
                            try
                            {
                                m_BlessedDate = reader.ReadDateTime();
                                // Validate the DateTime - if it's invalid, use MinValue
                                if (m_BlessedDate.Ticks < DateTime.MinValue.Ticks || m_BlessedDate.Ticks > DateTime.MaxValue.Ticks)
                                    m_BlessedDate = DateTime.MinValue;
                            }
                            catch
                            {
                                // If reading DateTime fails (corrupted data), use MinValue
                                m_BlessedDate = DateTime.MinValue;
                            }
                        }
                        else
                        {
                            m_BlessingReligion = VystiaReligion.None;
                            m_BlessingType = BlessingType.None;
                            m_BlessedDate = DateTime.MinValue;
                        }

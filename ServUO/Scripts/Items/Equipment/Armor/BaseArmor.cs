...
{
    public abstract class BaseArmor : Item, IScissorable, IFactionItem, ICraftable, IWearableDurability, IResource, ISetItem, IVvVItem, IOwnerRestricted, ITalismanProtection, IEngravable, IArtifact, ICombatEquipment, IQuality, IBlessedItem
    {
        #region Factions
        ...
        */
        ...
        public override void Deserialize(GenericReader reader)
        {
                ...
                case 8:
                    {
                        ...
                        // Vystia: Blessed item data (version 15+)
                        if (version >= 15)
                        {
            ...
            // Vystia: Initialize blessing fields for all versions (in case they weren't read)
            if (version < 15)
            {
                m_BlessingReligion = VystiaReligion.None;
                m_BlessingType = BlessingType.None;
            ...
            // Initialize m_BlessedBy for versions < 8 (it's only read in case 8+)
            if (version < 8)
            {
                m_BlessedBy = null;
            }

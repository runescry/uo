...
namespace Server.Items
{
    ...
    public class Spellbook : Item, ICraftable, ISlayer, IEngravable, IVvVItem, IOwnerRestricted, IWearableDurability, IFactionItem
	{
        ...
        public static SpellbookType GetTypeForSpell(int spellID)
		{
            ...
            {
                return SpellbookType.VystiaIceMage;  // Ice Magic (1000-1031) - Fixed: was 999-1030
            }
            else if (spellID >= 1032 && spellID <= 1063)
            {
                return SpellbookType.VystiaDruid;  // Nature Magic (1032-1063) - Fixed: was 1031-1062
            }
            else if (spellID >= 1064 && spellID <= 1095)
            {
                return SpellbookType.VystiaWitch;  // Hex Magic (1064-1095) - Fixed: was 1063-1094
            }
            else if (spellID >= 1096 && spellID <= 1127)
            {
                return SpellbookType.VystiaSorcerer;  // Elemental Magic (1096-1127) - Fixed: was 1095-1126
            }
            else if (spellID >= 1128 && spellID <= 1159)
            {
                return SpellbookType.VystiaWarlock;  // Dark Magic (1128-1159) - Fixed: was 1127-1158
            }
            else if (spellID >= 1160 && spellID <= 1191)
            {
                return SpellbookType.VystiaOracle;  // Divination Magic (1160-1191) - Fixed: was 1159-1190
            }
            else if (spellID >= 1192 && spellID <= 1223)
            {
                return SpellbookType.VystiaNecromancer;  // Necromancy (1192-1223) - Fixed: was 1191-1222
            }
            else if (spellID >= 1224 && spellID <= 1255)
            {
                return SpellbookType.VystiaSummoner;  // Summoning Magic (1224-1255) - Fixed: was 1223-1254
            }
            else if (spellID >= 1256 && spellID <= 1287)
            {
                return SpellbookType.VystiaShaman;  // Shamanic Magic (1256-1287) - Fixed: was 1255-1286
            }
			else if (spellID >= 1288 && spellID <= 1319)
			{
                return SpellbookType.VystiaBard;  // Bardic Magic (1288-1319)
            }
            else if (spellID >= 1384 && spellID <= 1415)
            {
                return SpellbookType.VystiaSongweaving;  // Songweaving (1384-1415)
            }
            else if (spellID >= 1320 && spellID <= 1351)
            {
                return SpellbookType.VystiaEnchanter;  // Enchanting Magic (1320-1351) - Fixed: was 1319-1350
            }
            else if (spellID >= 1352 && spellID <= 1383)
            {
                return SpellbookType.VystiaIllusionist;  // Illusion Magic (1352-1383) - Fixed: was 1351-1382
            }
		...
		{
			SpellbookType type = GetTypeForSpell(spellID);
			Console.WriteLine($"[FIND] Find({from.Name}, {spellID}) - GetTypeForSpell returned: {type}");
			return Find(from, spellID, type);
		}
		...
		{
			int originalSpellID = spellID;
			spellID -= BookOffset;

			bool inRange = (spellID >= 0 && spellID < BookCount);
			bool hasBit = inRange && (m_Content & ((ulong)1 << spellID)) != 0;

			Console.WriteLine($"[SERVER] HasSpell({originalSpellID}) - BookOffset: {BookOffset}, NormalizedID: {spellID}, InRange[0-{BookCount}]: {inRange}, HasBit: {hasBit}");

			return inRange && hasBit;
		}
		...
		public void DisplayTo(Mobile to)
		{
			...
			if (ObjectPropertyList.Enabled)
			{
				...
				else
				{
					if (ns.ContainerGridLines)
					{
						to.Send(new SpellbookContent6017(m_Count, offsetToSend, m_Content, this));
					}
					else
					{
						to.Send(new SpellbookContent(m_Count, offsetToSend, m_Content, this));
					}
				}
			}
			else
		...
		private static void EventSink_CastSpellRequest(CastSpellRequestEventArgs e)
		{
			...
			Console.WriteLine($"[SERVER] CastSpellRequest - Player: {from.Name}, SpellID: {spellID}, InitialBook: {(book != null ? $"0x{book.Serial:X8} ({book.GetType().Name})" : "NULL")}");

			if (book == null || !book.HasSpell(spellID))
			{
				Console.WriteLine($"[SERVER] Book is null or doesn't have spell. Searching for spellbook...");
				book = Find(from, spellID);
				Console.WriteLine($"[SERVER] Find() returned: {(book != null ? $"0x{book.Serial:X8} ({book.GetType().Name})" : "NULL")}");
			}

			if (book != null)
			{
				bool hasSpell = book.HasSpell(spellID);
				Console.WriteLine($"[SERVER] Book 0x{book.Serial:X8} HasSpell({spellID}): {hasSpell}");
				Console.WriteLine($"[SERVER] Book Details - Type: {book.SpellbookType}, BookOffset: {book.BookOffset}, BookCount: {book.BookCount}, Content: 0x{book.Content:X16}");
				...
				if (hasSpell)
				{
					...
					if (move != null)
					{
						Console.WriteLine($"[SERVER] Spell {spellID} is a SpecialMove");
						SpecialMove.SetCurrentMove(from, move);
					}
					else
					{
						Console.WriteLine($"[SERVER] Attempting to create spell from registry...");
						Spell spell = SpellRegistry.NewSpell(spellID, from, null);

						if (spell != null)
						{
							Console.WriteLine($"[SERVER] Spell created successfully: {spell.GetType().Name}");
							spell.Cast();
						}
						else
						{
							Console.WriteLine($"[SERVER] ERROR: SpellRegistry.NewSpell({spellID}) returned NULL");
							if ( !Server.Spells.SkillMasteries.MasteryInfo.IsPassiveMastery( spellID ) )
							{
								from.SendLocalizedMessage( 502345 ); // This spell has been temporarily disabled.
							}
						}
				...
				{
					Console.WriteLine($"[SERVER] FAIL: Book doesn't have spell {spellID}");
					from.SendLocalizedMessage(500015); // You do not have that spell!
				}
			}
			else
			{
				Console.WriteLine($"[SERVER] FAIL: No spellbook found for spell {spellID}");
				from.SendLocalizedMessage(500015); // You do not have that spell!
			}
		}
	}

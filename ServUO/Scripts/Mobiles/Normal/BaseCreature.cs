...
namespace Server.Mobiles
{
    ...
    public class BaseCreature : Mobile, IHonorTarget, IEngravable
    {
        ...
        public override void ComputeResistances()
        {
            ...
            // Vystia: Beastmaster + Frosthelm Faith: Pets +10% cold resist
            if (Controlled && ControlMaster is PlayerMobile pm)
            {
                int petColdResistBonus = Server.Custom.VystiaClasses.Systems.VystiaSkillIntegration.GetClassReligionSynergyPetResistBonus(pm, ResistanceType.Cold);
                if (petColdResistBonus > 0)
                {
                    Resistances[2] += petColdResistBonus; // Cold resistance
                }
            }
        }
        ...
        { 
            get 
            { 
                // Vystia: BeastBonding skill increases pet damage
                // +1% per 10 skill points (10% at GM)
                if (Controlled && m_ControlMaster != null && m_ControlMaster.Skills != null)
                {
                    double beastBonding = m_ControlMaster.Skills[SkillName.BeastBonding].Value;
                    if (beastBonding > 0)
                    {
                        return 1.0 + (beastBonding * 0.001); // 0.1% per point, 10% at GM
                    }

					// Vystia: Engineering skill increases construct pet damage
					// +1% per 10 skill points (10% at GM) for mechanical constructs
					if (this is BaseCreature && GetType().Name.Contains("Construct") || GetType().Name.Contains("Clockwork"))
					{
						double engineering = m_ControlMaster.Skills[SkillName.Engineering].Value;
						if (engineering > 0)
						{
							return 1.0 + (engineering * 0.001); // 0.1% per point, 10% at GM
						}
					}

					// Vystia: Conjuration skill increases summon pet damage
					// Scale from 0.8x at 0 skill to 1.5x at 100 skill
					if (m_ControlMaster is PlayerMobile pm)
					{
						// Check if this is a Vystia summoned pet (Summoner's pet - SummonedElemental or SummonedCreature)
						if (this is Server.Custom.VystiaClasses.Pets.VystiaSummonedPet vsp)
						{
							var petType = vsp.PetType;
							if (petType == Server.Custom.VystiaClasses.Pets.VystiaPetType.SummonedElemental || 
							    petType == Server.Custom.VystiaClasses.Pets.VystiaPetType.SummonedCreature)
							{
								return Server.Custom.VystiaClasses.Systems.VystiaSkillIntegration.GetConjurationPetDamageBonus(pm);
							}
						}
					}
                }
...
*/
        ...
        public override int HitsMax
        {
            get
            {
                ...
                // Vystia: Summoner + Celestis Arcanum: Summon HP +15%
                // Check if this is a summoned pet (controlled by a Summoner)
                if (Controlled && ControlMaster is PlayerMobile pm)
                {
                    double petHPBonus = Server.Custom.VystiaClasses.Systems.VystiaSkillIntegration.GetClassReligionSynergyPetHPBonus(pm);
                    if (petHPBonus > 0)
                    {
                        value = (int)(value * (1.0 + petHPBonus)); // +15% HP
                    }
                }
        ...
        */
        ...
        {
			// Vystia: Clear threat table when creature dies
			Server.Custom.VystiaClasses.Systems.ThreatSystem.ClearThreat(this);
        ...
        */
        ...
        private void ApplyDragonSpeedIfControlled()
        {
            if (!IsDragonSpeedType(this))
            {
                return;
            ...
            if (Controlled)
            {
                ForceActiveSpeed = 0.05;
                ForcePassiveSpeed = 0.05;
        ...
        private static bool IsDragonSpeedType(BaseCreature creature)
        {
            if (creature == null)
            {
                return false;
            ...
            var name = creature.GetType().Name;
            return name.IndexOf("Dragon", StringComparison.OrdinalIgnoreCase) >= 0
                || name.IndexOf("Drake", StringComparison.OrdinalIgnoreCase) >= 0
                || name.IndexOf("Wyrm", StringComparison.OrdinalIgnoreCase) >= 0;
        }
        ...
        public virtual void Heal(Mobile patient)
        {
            ...
            else
            {
                ...
                if (chance > Utility.RandomDouble())
                {
					...
					// Vystia: DivineGrace healing power bonus (+0.5% per 10 skill, 50% at GM)
					// Note: This should check the patient, not the healer, but BaseCreature doesn't have access to patient here
					// This code may need to be moved to a different location where patient is available

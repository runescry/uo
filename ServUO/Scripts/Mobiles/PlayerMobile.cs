...
namespace Server.Mobiles
{
	...
	public partial class PlayerMobile : Mobile, IHonorTarget
	{
		...
		private Server.Custom.VystiaClasses.PlayerClassType m_VystiaClass; // Vystia custom class system (v1)
		private Server.Custom.VystiaClasses.PlayerClassTypeV2 m_VystiaClassV2; // Vystia class system (v2)
		private int m_SongMasteryLevel;
		private int m_SongMasteryXP;
		private int m_SongMasteryPoints;
		private Dictionary<string, int> m_SongMasteryPotency;
		private Dictionary<string, int> m_SongMasteryDuration;
		private int m_Concentration;
		private Dictionary<string, int> m_ActiveSongConcentration;
		...
		*/
		...
		[CommandProperty(AccessLevel.GameMaster)]
		public Server.Custom.VystiaClasses.PlayerClassType VystiaClass { get { return m_VystiaClass; } set { m_VystiaClass = value; } }

		[CommandProperty(AccessLevel.GameMaster)]
		public Server.Custom.VystiaClasses.PlayerClassTypeV2 VystiaClassV2 { get { return m_VystiaClassV2; } set { m_VystiaClassV2 = value; } }

		public int SongMasteryLevel { get { return m_SongMasteryLevel; } set { m_SongMasteryLevel = value; } }
		public int SongMasteryXP { get { return m_SongMasteryXP; } set { m_SongMasteryXP = value; } }
		public int SongMasteryPoints { get { return m_SongMasteryPoints; } set { m_SongMasteryPoints = value; } }
		public Dictionary<string, int> SongMasteryPotency
		{
			get { return m_SongMasteryPotency ?? (m_SongMasteryPotency = new Dictionary<string, int>(StringComparer.OrdinalIgnoreCase)); }
			set { m_SongMasteryPotency = value; }
		}

		public Dictionary<string, int> SongMasteryDuration
		{
			get { return m_SongMasteryDuration ?? (m_SongMasteryDuration = new Dictionary<string, int>(StringComparer.OrdinalIgnoreCase)); }
			set { m_SongMasteryDuration = value; }
		}

		public int Concentration { get { return m_Concentration; } set { m_Concentration = value; } }
		public int MaxConcentration { get { return 100; } }

		public Dictionary<string, int> ActiveSongConcentration
		{
			get { return m_ActiveSongConcentration ?? (m_ActiveSongConcentration = new Dictionary<string, int>(StringComparer.OrdinalIgnoreCase)); }
			set { m_ActiveSongConcentration = value; }
		}
        ...
        public override void ComputeResistances()
        {
            ...
            // Vystia: Apply religion passive resistance bonuses
            if (this is PlayerMobile pm)
            {
                Resistances[0] += Server.Custom.VystiaClasses.Systems.VystiaSkillIntegration.GetReligionResistanceBonus(pm, ResistanceType.Physical);
                Resistances[1] += Server.Custom.VystiaClasses.Systems.VystiaSkillIntegration.GetReligionResistanceBonus(pm, ResistanceType.Fire);
                Resistances[2] += Server.Custom.VystiaClasses.Systems.VystiaSkillIntegration.GetReligionResistanceBonus(pm, ResistanceType.Cold);
                Resistances[3] += Server.Custom.VystiaClasses.Systems.VystiaSkillIntegration.GetReligionResistanceBonus(pm, ResistanceType.Poison);
                Resistances[4] += Server.Custom.VystiaClasses.Systems.VystiaSkillIntegration.GetReligionResistanceBonus(pm, ResistanceType.Energy);

                // Vystia: Cogsmith Creed Initiate: Armor +5
                int armorBonus = Server.Custom.VystiaClasses.Systems.VystiaSkillIntegration.GetReligionArmorBonus(pm);
                if (armorBonus > 0)
                {
                    // Add to all resistances as armor bonus
                    for (int i = 0; i < Resistances.Length; ++i)
                    {
                        Resistances[i] += armorBonus;
                    }
                }
            }
		...
		private static void OnLogin(LoginEventArgs e)
		{
            ...
            if (from is PlayerMobile pm &&
                pm.VystiaClassV2 == Server.Custom.VystiaClasses.PlayerClassTypeV2.Bard)
            {
		...
		public override int HitsMax
		{
			get
			{
				...
				if (Core.AOS)
				{
                    ...
                    // Vystia: Blessed item HP bonus (Frosthelm Faith)
                    strOffs += Server.Custom.VystiaClasses.Systems.VystiaSkillIntegration.GetBlessedItemHPBonus(this);

					// Vystia: Religion HP bonus
					strOffs += Server.Custom.VystiaClasses.Systems.VystiaSkillIntegration.GetReligionHPBonus(this);
				}
				else
				{
					strBase = RawStr;
		...
		// Vystia: Override DefaultManaRegen to include religion bonus
		// Note: DefaultManaRegen is virtual in BaseCreature, but PlayerMobile extends Mobile
		// We'll use a different approach - override ManaMax or use a property
		public virtual int DefaultManaRegen
		{
			get
			{
				int regen = 0; // Base Mobile doesn't have DefaultManaRegen, start at 0
				// Vystia: Religion mana regen bonus (Celestis Arcanum +2 at Devoted)
				regen += Server.Custom.VystiaClasses.Systems.VystiaSkillIntegration.GetReligionManaRegenBonus(this);

				// Vystia: Wizard + Celestis Arcanum: Mana regen +10%
				int synergyBonus = Server.Custom.VystiaClasses.Systems.VystiaSkillIntegration.GetClassReligionSynergyManaRegenBonus(this);
				if (synergyBonus > 0)
				{
					regen = (int)(regen * (1.0 + (synergyBonus / 100.0))); // synergyBonus is 10, so +10%
				}
		...
		// Vystia: Override MutateSkill to apply class-religion synergy skill bonuses
		public override void MutateSkill(SkillName skill, ref double value)
		{
			base.MutateSkill(skill, ref value);

			// Oracle + Celestis Arcanum: +5 Divination skill
			double skillBonus = Server.Custom.VystiaClasses.Systems.VystiaSkillIntegration.GetClassReligionSynergySkillBonus(this, skill);
			if (skillBonus > 0)
			{
				value += skillBonus; // Add +5 to skill value
			}
		}
		...
		{
			int mana = base.ManaMax + AosAttributes.GetValue(this, AosAttribute.BonusMana) +
				   ((Core.ML && Race == Race.Elf) ? 20 : 0) +
                   MasteryInfo.IntuitionBonus(this) +
                   UraliTranceTonic.GetManaBuff(this);

			// Vystia: Blessed item Mana bonus (Surya's Sandscript)
			mana += Server.Custom.VystiaClasses.Systems.VystiaSkillIntegration.GetBlessedItemManaBonus(this);

			return mana;
		} }

		[CommandProperty(AccessLevel.GameMaster)]
		public new int FollowersMax
		{
			get
			{
				int baseMax = base.FollowersMax;
				
				// Vystia: BeastBonding skill grants +1 control slot at GM (100.0)
				if (Skills != null && Skills[SkillName.BeastBonding].Value >= 100.0)
				{
					return baseMax + 1;
				}
		...
		public override void OnDamage(int amount, Mobile from, bool willKill)
		{
			...
			// Check for Ice Shield damage reflection (physical damage only)
			if (from != null && from is Mobile attacker && amount > 0)
			{
				Server.Spells.VystiaSpells.IceMage.IceShieldReflectionEffect.CheckReflection(this, attacker, amount);
			}

			// Vystia: Generate secondary resources from damage taken (Fortitude, etc.)
			if (from != null && amount > 0)
			{
				var manager = Server.Custom.VystiaClasses.Systems.VystiaResourceManager.GetManager(this);
				manager?.OnDamageTaken(from, amount);
			}
		...
		public override void OnDeath(Container c)
		{
            ...
            // Vystia: PvP kill rewards (piety and faction reputation)
            if (killer != null && killer != this && killer.Alive)
            {
                // Religion PvP: Award piety for killing opposed religion player
                var killerReligion = Server.Custom.VystiaClasses.Religion.VystiaPiety.GetReligion(killer);
                var victimReligion = Server.Custom.VystiaClasses.Religion.VystiaPiety.GetReligion(this);
                
                if (killerReligion != Server.Custom.VystiaClasses.Religion.VystiaReligion.None &&
                    victimReligion != Server.Custom.VystiaClasses.Religion.VystiaReligion.None &&
                    Server.Custom.VystiaClasses.Religion.VystiaReligionSystem.AreReligionsOpposed(killerReligion, victimReligion))
                {
                    Server.Custom.VystiaClasses.Religion.VystiaPiety.AddPiety(killer, 15, $"PvP kill: {this.Name}");
                }

                // Faction PvP: Award reputation for killing enemy faction player
                // TODO: Determine faction alignment from player's region/class and award reputation
                // This requires faction assignment system to be implemented
            }
		...
		public override void Deserialize(GenericReader reader)
		{
			...
			switch (version)
			{
                case 43: // Version 43, Bard Concentration
                    {
                        var binaryReader = reader as Server.BinaryFileReader;
                        long startPos = binaryReader != null ? binaryReader.Position : -1;

                        if (!TryReadSongMasteryBlock(reader, SongMasteryReadMode.ConcentrationLast))
                        {
                            if (binaryReader != null && startPos >= 0)
                            {
                                binaryReader.Seek(startPos, SeekOrigin.Begin);
                                if (!TryReadSongMasteryBlock(reader, SongMasteryReadMode.ConcentrationFirst))
                                {
                                    if (binaryReader != null && startPos >= 0)
                                    {
                                        binaryReader.Seek(startPos, SeekOrigin.Begin);
                                    ...
                                    if (!TryReadSongMasteryBlock(reader, SongMasteryReadMode.NoConcentration))
                                    {
                                        m_SongMasteryPotency = null;
                                        m_SongMasteryDuration = null;
                                        m_Concentration = MaxConcentration;
                                    }
                                }
                            }
                        }
                ...
                case 42: // Version 42, Song Mastery progression
                    {
                        m_SongMasteryLevel = reader.ReadInt();
                        m_SongMasteryXP = reader.ReadInt();
                        m_SongMasteryPoints = reader.ReadInt();

                        int potencyCount = reader.ReadInt();
                        if (potencyCount > 0)
                        {
                            m_SongMasteryPotency = new Dictionary<string, int>(StringComparer.OrdinalIgnoreCase);
                            for (int i = 0; i < potencyCount; i++)
                            {
                                string key = reader.ReadString();
                                int value = reader.ReadInt();
                                if (!string.IsNullOrEmpty(key))
                                {
                                    m_SongMasteryPotency[key] = value;
                                }
                            }
                        }

                        int durationCount = reader.ReadInt();
                        if (durationCount > 0)
                        {
                            m_SongMasteryDuration = new Dictionary<string, int>(StringComparer.OrdinalIgnoreCase);
                            for (int i = 0; i < durationCount; i++)
                            {
                                ...
                                if (!string.IsNullOrEmpty(key))
                                {
                                    m_SongMasteryDuration[key] = value;
                                }
                            }
                        }

                        if (m_Concentration <= 0 || m_Concentration > MaxConcentration)
                            m_Concentration = MaxConcentration;
                        goto case 41;
                    }
                case 41: // Version 41, Vystia class system v2
				...
				case 16:
					{
						...
						m_VystiaClass = (Server.Custom.VystiaClasses.PlayerClassType)reader.ReadEncodedInt(); // Vystia custom class system (v1)
						m_VystiaClassV2 = version >= 41
							? (Server.Custom.VystiaClasses.PlayerClassTypeV2)reader.ReadEncodedInt()
							: Server.Custom.VystiaClasses.PlayerClassTypeV2.None;

						if (m_VystiaClassV2 == Server.Custom.VystiaClasses.PlayerClassTypeV2.None &&
							m_VystiaClass != Server.Custom.VystiaClasses.PlayerClassType.None)
						{
							if (Enum.TryParse(m_VystiaClass.ToString(), true, out Server.Custom.VystiaClasses.PlayerClassTypeV2 legacyMapped))
								m_VystiaClassV2 = legacyMapped;
						}
						goto case 15;
					}
				case 15:
			...
			if (m_VystiaClassV2 != Server.Custom.VystiaClasses.PlayerClassTypeV2.None)
			{
				Server.Custom.VystiaClasses.VystiaClassManager.Instance.RestoreClass(this);
			}
		}

        private enum SongMasteryReadMode
        {
            ConcentrationLast,
            ConcentrationFirst,
            NoConcentration
        ...
        private bool TryReadSongMasteryBlock(GenericReader reader, SongMasteryReadMode mode)
        {
            int concentration = 0;

            if (mode == SongMasteryReadMode.ConcentrationFirst)
            {
            ...
            int potencyCount = reader.ReadInt();
            if (potencyCount < 0 || potencyCount > 1000)
            {
                return false;
            }
            ...
            int durationCount = reader.ReadInt();
            if (durationCount < 0 || durationCount > 1000)
            {
                return false;
            }
            ...
            if (mode == SongMasteryReadMode.ConcentrationLast)
            {
                concentration = reader.ReadInt();
            }
            else if (mode == SongMasteryReadMode.NoConcentration)
            {
		...
		public override void Serialize(GenericWriter writer)
		{
			...
			writer.Write(m_SongMasteryLevel);
			writer.Write(m_SongMasteryXP);
			writer.Write(m_SongMasteryPoints);

			if (m_SongMasteryPotency == null)
			{
				writer.Write(0);
			}
			else
			{
				writer.Write(m_SongMasteryPotency.Count);
				foreach (var entry in m_SongMasteryPotency)
				{
			...
			if (m_SongMasteryDuration == null)
			{
				writer.Write(0);
			}
			else
			{
				writer.Write(m_SongMasteryDuration.Count);
				foreach (var entry in m_SongMasteryDuration)
				{
					writer.Write(entry.Key);
					writer.Write(entry.Value);
				}
			}
			writer.Write(m_Concentration);
			...
			writer.WriteEncodedInt((int)m_VystiaClass); // Vystia custom class system (v1)
			writer.WriteEncodedInt((int)m_VystiaClassV2); // Vystia class system (v2)
		...
		public override int ComputeMovementSpeed(Direction dir, bool checkTurning)
		{
			...
			{
				int baseSpeed = (running ? RunMount : WalkMount);
				return ApplyMovementSpeedBuffs(baseSpeed);
			}

			int footSpeed = (running ? RunFoot : WalkFoot);
			return ApplyMovementSpeedBuffs(footSpeed);
		}

		private int ApplyMovementSpeedBuffs(int baseSpeed)
		{
			double modifier = 1.0;
			var buffs = Server.Custom.VystiaClasses.Systems.VystiaBuffManager.Instance.GetAllBuffs(this);
			foreach (var buff in buffs)
			{
				if (buff.Definition.Type == Server.Custom.VystiaClasses.Systems.VystiaBuffType.MovementSpeedBuff ||
					buff.Definition.Type == Server.Custom.VystiaClasses.Systems.VystiaBuffType.SongOfSwiftness)
				{
					if (buff.SpeedModifier > 0)
					{
						modifier *= buff.SpeedModifier;
					}
				}
			}
			...
			int adjusted = (int)Math.Max(1, Math.Round(baseSpeed / modifier));
			return adjusted;
		}

...
namespace Server.Mobiles
{
	...
	public abstract class BaseAI
	{
		...
		public virtual bool HandlesOnSpeech(Mobile from)
		{
			...
			// Check if this NPC supports LLM conversation (allow it even outside towns)
			if (from.Alive && from is PlayerMobile && 
				Server.Services.LLM.LLMConversationPlugin.IsEnabled() &&
				m_Mobile is Server.Services.LLM.ILLMConversational llmNpc && 
				llmNpc.LLMConversationEnabled)
			{
				if (from.InRange(m_Mobile.Location, llmNpc.HearingRange))
				{
					return true; // LLM NPCs can hear speech within their hearing range
				}
			}
		...
		public virtual void OnSpeech(SpeechEventArgs e)
		{
			...
			// We'll check and process asynchronously after speech is displayed
			// Do a VERY fast check first (just interface check, no expensive operations)
			// Even this check is deferred to avoid any delay
			if (!e.Handled)
			{
				...
				// Process asynchronously AFTER speech display completes
				// Use DelayCall to ensure speech packets are sent first
				// The speech packet is sent AFTER OnSpeech returns, so we need to defer everything
				// Use minimal delay - just enough for packet to be sent
				Timer.DelayCall(TimeSpan.Zero, () =>
				{
					...
					if (pluginEnabled)
					{
						...
						{
							// Re-check if we should handle (this includes the expensive GetMobilesInRange check)
							if (npc != null && !npc.Deleted && player != null && !player.Deleted)
							{
								...
								if (shouldHandle)
								{
									...
									SpeechEventArgs newArgs = new SpeechEventArgs(player, speech, MessageType.Regular, player.SpeechHue, new int[0]);
        ...
        */
		...
		public virtual bool AcquireFocusMob(int iRange, FightMode acqType, bool bPlayerOnly, bool bFacFriend, bool bFacFoe)
		{
			...
			if (map != null)
			{
				...
				// Vystia: Check threat system for highest threat target
				Mobile highestThreat = Server.Custom.VystiaClasses.Systems.ThreatSystem.GetHighestThreat(m_Mobile as BaseCreature);
				if (highestThreat != null && highestThreat.Alive && !highestThreat.Deleted && 
					m_Mobile.CanSee(highestThreat) && m_Mobile.InLOS(highestThreat) &&
					m_Mobile.InRange(highestThreat, iRange))
				{
					newFocusMob = highestThreat;
					val = double.MaxValue; // Prioritize highest threat
				}
				...
				foreach (var m in eable)
				{
					...
					// Vystia: Skip if we already have a higher threat target
					if (newFocusMob == highestThreat && m != highestThreat)
					{
						continue;
					}

...
namespace Server.Mobiles
{
    public abstract class BaseHealer : BaseVendor
    {
        ...
        public virtual void OfferResurrection(Mobile m)
        {
            ...
            // Vystia: Apply resurrection fee (50g for non-young players)
            int resurrectionFee = VystiaServiceFees.GetResurrectionFee(m);

            if (resurrectionFee > 0)
            {
                // Use the price-based resurrection gump
                m.SendGump(new ResurrectGump(m, this, resurrectionFee));
            }
            else
            {
                // Free resurrection for young players

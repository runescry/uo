...
namespace Server.Engines.Craft
{
    ...
    public class CraftItem
    {
		...
		public double GetExceptionalChance(CraftSystem system, double chance, Mobile from)
		{
			...
			// Vystia: Forge Blessing devotion power (+10% exceptional chance)
			if (from is PlayerMobile pm && Server.Custom.VystiaClasses.Religion.VystiaReligionSystem.HasActiveDevotionPower(pm, "Forge Blessing"))
			{
				bonus += 0.10; // +10% exceptional chance
			}
        ...
        public double GetSuccessChance(Mobile from, Type typeRes, CraftSystem craftSystem, bool gainSkills, ref bool allRequiredSkills, int maxAmount)
		{
			...
			// Vystia: Engineering craft success bonus for Tinkering (+0.5% per point)
			if (allRequiredSkills && craftSystem.MainSkill == SkillName.Tinkering && from != null && from.Skills != null)
			{
				double engineering = from.Skills[SkillName.Engineering].Value;
				if (engineering > 0)
				{
					double craftBonus = engineering * 0.005; // +0.5% per point
					chance += craftBonus;
				}
			}

			// Vystia: Religion skill bonuses (e.g., Cogsmith Creed +3/+5 crafting)
			if (allRequiredSkills && from is PlayerMobile pm)
			{
				double religionBonus = Server.Custom.VystiaClasses.Systems.VystiaSkillIntegration.GetReligionSkillBonus(pm, craftSystem.MainSkill);
				if (religionBonus > 0.0)
				{
					// Convert skill bonus to success chance bonus (1 skill point = 1% success chance)
					chance += religionBonus / 100.0;
				}
			}

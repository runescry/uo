...
namespace Server.Engines.Quests
{
    ...
    public class MondainQuestGump : BaseQuestGump
    {
        ...
        private const int VystiaButtonOffset = 2000;
        ...
        private readonly List<int> m_VystiaActiveQuestIds;
            ...
            : base(75, 25)
        {
            ...
            // Capture active Vystia DynamicQuests so we can show them in the standard Quest Log.
            m_VystiaActiveQuestIds = new List<int>();
            if (m_From != null)
            {
                var tracker = VystiaQuestTracker.GetTracker(m_From);
                if (tracker != null)
                {
                    foreach (int qid in tracker.GetActiveQuests())
                        m_VystiaActiveQuestIds.Add(qid);
                }
            }
        ...
        public virtual void SecMain()
        {
            ...
            // Add Vystia DynamicQuests below the built-in quest list (if any).
            if (m_VystiaActiveQuestIds != null && m_VystiaActiveQuestIds.Count > 0)
            {
                offset += 8;
                AddHtmlObject(98, offset, 270, 21, "Vystia Quests", White, false, false);
                offset += 21;

                for (int i = 0; i < m_VystiaActiveQuestIds.Count; i++)
                {
                    int qid = m_VystiaActiveQuestIds[i];
                    var dq = DynamicQuestManager.GetQuest(qid);
                    string title = dq != null ? dq.Title : ("Vystia Quest #" + qid);

                    AddHtmlObject(98, offset, 270, 21, title, White, false, false);
                    AddButton(368, offset, 0x26B0, 0x26B1, VystiaButtonOffset + i, GumpButtonType.Reply, 0);
                    offset += 21;
                }
            }
        ...
        { 
            ...
            { 
                ...
                case (int)Buttons.CompleteQuest:
                    ...
                    // show quest
                default:
                    if (m_Section != Section.Main)
                        break;

                    // Built-in quest list
                    if (info.ButtonID >= ButtonOffset && info.ButtonID < m_From.Quests.Count + ButtonOffset)
                    {
                    ...
                    // Vystia quest list
                    if (info.ButtonID >= VystiaButtonOffset && m_VystiaActiveQuestIds != null)
                    {
                        int idx = info.ButtonID - VystiaButtonOffset;
                        if (idx >= 0 && idx < m_VystiaActiveQuestIds.Count)
                        {
                            int qid = m_VystiaActiveQuestIds[idx];
                            m_From.SendGump(new VystiaQuestOfferGump(m_From, qid, false));
                        }
                    }
                    break;
            }

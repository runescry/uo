...
namespace Server.Gumps
{
	...
	{
		private class VystiaQuestLogEntry
		{
			public int QuestId;
			...
			switch ( info.ButtonID )
			{
				...
				default:
				{
				    if ( info.ButtonID >= 2000 )
                    {
						...
						else if (m_List[index] is VystiaQuestLogEntry)
						{
							VystiaQuestLogEntry ve = m_List[index] as VystiaQuestLogEntry;

							if (ve != null && m_From is PlayerMobile)
							{
								m_From.SendGump(new XMLQuestLogGump(m_From, m_Page, null));
								m_From.SendGump(new VystiaQuestOfferGump((PlayerMobile)m_From, ve.QuestId, false));
							}
						}
                    } 
		...
		public XMLQuestLogGump( Mobile from, int page, List<object> list ) : base( 12, 24 )
		{
			...
			if ( list == null )
			{
				...
				// Add Vystia DynamicQuests FIRST so they always show on page 1 (instead of being pushed to later pages
				// by large XmlQuest/QuestBook/completed lists).
				int vystiaActiveCount = 0;
				if (from is PlayerMobile)
				{
					PlayerMobile pm = (PlayerMobile)from;
					VystiaQuestTracker tracker = VystiaQuestTracker.GetTracker(pm);

					if (tracker != null)
					{
						foreach (int questId in tracker.GetActiveQuests())
						{
							list.Add(new VystiaQuestLogEntry { QuestId = questId, Completed = false });
							vystiaActiveCount++;
						}
					}
				}
			...
			// Visible sanity check: show how many active Vystia quests we detected for this player.
			if (from is PlayerMobile)
			{
				PlayerMobile pm = (PlayerMobile)from;
				VystiaQuestTracker tracker = VystiaQuestTracker.GetTracker(pm);
				int vystiaCount = tracker != null ? tracker.GetActiveQuests().Count : 0;
				AddLabel( 20, 34, 1152, $"Vystia Active: {vystiaCount}" );
			}
			...
			for ( int i = index; i < (index + count) && i >= 0 && i < list.Count; ++i )
			{
					...
					if(obj is XmlQuestPoints.QuestEntry)
				{
					...
					if (obj is VystiaQuestLogEntry)
				{
					VystiaQuestLogEntry e = (VystiaQuestLogEntry)obj;
					int y = 96 + (tableIndex++ * 32);

					int color = 5;
					DynamicQuest dq = DynamicQuestManager.GetQuest(e.QuestId);
					string name = dq != null ? dq.Title : ("Vystia Quest #" + e.QuestId);

					AddButton( 60 - xoffset, y + 2, 0xFAB, 0xFAD, 2000 + i, GumpButtonType.Reply, 0 ); // open gump
					AddLabel( 100 - xoffset, y, color, name );
					AddLabel( 270 - xoffset, y, color, "Vystia" );
					AddLabel( 315 - xoffset, y, color, "In Progress" );
					AddLabel( 400 - xoffset, y, color, "" );
					AddLabel( 515 - xoffset, y, color, "" );
					AddLabel( 560 - xoffset, y, color, "" );
					AddLabel( 741 - xoffset, y, color, "" );
				}
			}
		}
	}

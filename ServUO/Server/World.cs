...
namespace Server
{
	public static class World
	{
		...
		public static void Load()
		{
			...
			int failedTypeID = 0;
			int skippedItems = 0; // Track automatically skipped items
				...
				using (FileStream bin = new FileStream(ItemDataPath, FileMode.Open, FileAccess.Read, FileShare.Read))
				{
					...
					for (int i = 0; i < items.Count; ++i)
					{
						...
						if (item != null)
						{
							...
							{
								// Auto-skip problematic items and continue loading
								Utility.PushColor(ConsoleColor.Yellow);
								Console.WriteLine("Auto-skipping problematic item: {0} (Serial: 0x{1:X8}) - {2}", 
									item.GetType().Name, item.Serial.Value, e.Message);
								Utility.PopColor();
								...
								// Continue loading instead of breaking
								continue;
							}
						}
					}
			...
			{
				Utility.PushColor(ConsoleColor.Yellow);
				Console.WriteLine("Automatically skipped {0} problematic item(s) during load.", skippedItems);
				Utility.PopColor();
			}

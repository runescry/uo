...
namespace Server.Network
{
	...
	public static class PacketHandlers
	{
		...
		public static void TextCommand(NetState state, PacketReader pvSrc)
		{
			...
			Console.WriteLine($"[TEXTCOMMAND] Packet 0x12 received - Type: 0x{type:X2}, Command: '{command}'");
				...
				case 0x27: // Cast spell from book
					{
						...
						Console.WriteLine($"[PACKET 0x27] Raw command string: '{command}'");
						Console.WriteLine($"[PACKET 0x27] Split parts: {split.Length}");
					...
					if (split.Length > 0)
					{
						...
						Console.WriteLine($"[PACKET 0x27] Parsed - Raw SpellID: {rawSpellID}, Final SpellID: {spellID}, Serial: {serial} (0x{serial:X8})");

							Item spellbook = World.FindItem(serial);
							Console.WriteLine($"[PACKET 0x27] World.FindItem({serial}) returned: {(spellbook != null ? $"0x{spellbook.Serial:X8} ({spellbook.GetType().Name})" : "NULL")}");

							EventSink.InvokeCastSpellRequest(new CastSpellRequestEventArgs(m, spellID, spellbook));
						}
		...
		public static void CastSpell(NetState state, PacketReader pvSrc)
		{
			...
			Console.WriteLine($"[CASTSPELL] Packet 0x1C received");
			...
			int flag = pvSrc.ReadInt16();
			Console.WriteLine($"[CASTSPELL] Flag: {flag}");

			if (flag == 1)
			{
				int serial = pvSrc.ReadInt32();
				spellbook = World.FindItem(serial);
				Console.WriteLine($"[CASTSPELL] Reading spellbook serial: {serial} (0x{serial:X8}), Found: {(spellbook != null ? spellbook.GetType().Name : "NULL")}");
			}
